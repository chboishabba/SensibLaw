...
-- Bounded factor-composition generation --------------------------------------
--
-- The v1 generator ranked the complete pair relation and only then retained the
-- first K candidates per bridge.  Output cardinality was bounded, but work was
-- still quadratic in bridge degree.  Generate the deterministic first K pairs
-- inside a LATERAL subquery instead, and record when a bridge had additional
-- possible pairs.  Overflow is an execution receipt, never semantic rejection.
...
-- These access paths match the bounded LATERAL pair enumerator.  The previous
-- object index ordered role before factor, which is useful for role lookup but
-- less useful for deterministic factor-id pair generation.
...
BEGIN
    ...
    -- Local-object bridges ----------------------------------------------------
    -- Discover bridges linearly, then ask each bridge for at most K ordered
    -- pairs.  PostgreSQL can stop the indexed object/factor scan at the LIMIT
    -- instead of materialising every n(n-1)/2 pair merely to rank them.
    WITH bridge AS MATERIALIZED (
        ...
        SELECT bridge.region_id,
               ...
               bridge.object_id,
               chosen.left_factor_id,
               chosen.right_factor_id,
               chosen.left_role_symbol_id,
               chosen.right_role_symbol_id,
    ...
    SELECT selected_run_id,
           ...
           NULL,
           NULL,
           bridge.participant_count,
           (bridge.participant_count * (bridge.participant_count - 1)) / 2,
           max_per_bridge::SMALLINT
      FROM bridge
     WHERE (bridge.participant_count * (bridge.participant_count - 1)) / 2
           > max_per_bridge
    ...
    -- Proof-relevant entity bridges ------------------------------------------
    -- Identity projections are already sparse.  Still apply the same bounded
    -- pair generation contract and expose overflow instead of ranking a complete
    -- projected-edge Cartesian product.
    ...
    RETURN inserted_count + identity_inserted_count;
END;
$$;
