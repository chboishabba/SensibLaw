...
-- Adjacent sentence/paragraph regions (kinds 2 and 4) are overlapping evidence
-- fibres, not canonical containment parents. They own adjacent reconciliation
-- evidence and must never be routed through the canonical parent-frontier
-- reducer. Preserve ordinary sparse-frontier close behaviour for canonical
-- parent kinds while retaining the hierarchy deferred-publication boundary.
...
BEGIN
    ...
    -- Overlapping evidence fibres are reconciled by
    -- execute_numeric_pnf_adjacent_work(); they are not parent reductions.
    IF NEW.region_kind IN (2, 4) THEN
        RETURN NEW;
    END IF;
...
-- The document coordinator likewise owns only canonical containment-frontier
-- interfaces. Exclude pair/evidence fibres from both dirty seeding and upward
-- affected-key closure by excluding them from the eligible carrier.
...
COMMENT ON FUNCTION execution.reduce_numeric_pnf_interface_on_close() IS
    'Sparse frontier close trigger for canonical containment interfaces only; adjacent sentence/paragraph evidence fibres (kinds 2/4) remain owned by adjacent reconciliation.';
COMMENT ON FUNCTION execution.reduce_numeric_pnf_document_frontiers(TEXT, TEXT) IS
    'Delta-fed canonical frontier coordinator excluding overlapping adjacent evidence fibres (kinds 2/4) from dirty seeding, upward closure, and parent reduction.';
