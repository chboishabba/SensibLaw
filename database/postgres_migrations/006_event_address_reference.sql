-- Layer 4 extension: structured event addresses (PostgreSQL)
-- Depends on: 004_layer4_events_and_remedies.sql
BEGIN;
...
JOIN protected_interest_type pit ON pit.id = h.protected_interest_type_id
LEFT JOIN wrong_type wt ON wt.id = e.wrong_type_id
GROUP BY pit.id, pit.description, COALESCE(wt.legal_system_id, e.legal_system_id), e.address_id;
