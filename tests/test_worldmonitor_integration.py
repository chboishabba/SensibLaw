...
def test_worldmonitor_import_normalizes_fixtures_to_units_and_queries(
    tmp_path: Path,
) -> None:
    ...
    with sqlite3.connect(itir_db) as conn:
        ...
        summary_payload = build_worldmonitor_capture_summary(
            conn, import_run_id="worldmonitor-test-v1"
        )
        assert summary_payload["captureCount"] == 7
        assert summary_payload["labelCoverage"]["withText"] == 7

        captures = query_worldmonitor_captures(
            conn, import_run_id="worldmonitor-test-v1", limit=10
        )
        assert captures
        assert captures[0]["importRunId"] == "worldmonitor-test-v1"
        assert captures[0]["sourceFile"] == str(source_dir / "gamma-irradiators.json")

        rows = load_worldmonitor_activity_rows(
            conn, date=captures[0]["capturedDate"], limit=10
        )
        assert rows
        assert any(row["kind"] == "worldmonitor_capture" for row in rows)

        chronology_payload = build_worldmonitor_chronology(
            conn, import_run_id="worldmonitor-test-v1"
        )
    ...
    units = load_worldmonitor_units(
        itir_db, import_run_id="worldmonitor-test-v1", limit=3
    )
    assert len(units) == 3
...
def test_worldmonitor_import_falls_back_to_stable_timestamp_without_extracted_date(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "worldmonitor"
    source_dir.mkdir(parents=True, exist_ok=True)
...
def test_worldmonitor_import_keeps_same_row_id_distinct_across_files(
    tmp_path: Path,
) -> None:
    source_dir = tmp_path / "worldmonitor"
    source_dir.mkdir(parents=True, exist_ok=True)
