...
from src.fact_intake.handoff_artifacts import (
    resolve_handoff_artifact_metadata,
    write_handoff_artifact,
)


def test_resolve_handoff_artifact_metadata_handles_personal_and_protected_modes() -> (
    None
):
...
def test_write_handoff_artifact_emits_normalized_report_and_summary(tmp_path) -> None:
    ...
    normalized = json.loads(
        (tmp_path / "normalized_input.json").read_text(encoding="utf-8")
    )
    report = json.loads(
        (tmp_path / "personal.handoff.report.v1.json").read_text(encoding="utf-8")
    )
