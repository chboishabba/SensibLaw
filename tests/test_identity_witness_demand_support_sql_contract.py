...
M111 = ROOT / "database/postgres_migrations/111_identity_witness_demand_support_projection.sql"
...
def test_accepted_projection_requires_explicit_admission() -> None:
    sql = M111.read_text(encoding="utf-8")
    accepted = sql.split(
        "CREATE OR REPLACE VIEW execution.semantic_pnf_accepted_identity_witness_demand_support_v1", 1
    )[1].split(";", 1)[0]
