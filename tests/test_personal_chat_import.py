...
from scripts.build_personal_handoff_from_chat_json import (
    build_handoff_from_chat_artifact,
    main,
)
...
_PERSONAL_FIXTURE_PATH = (
    ...
    / "fixtures"
    / "fact_intake"
    / "personal_chat_input_v1.json"
)
_PROTECTED_FIXTURE_PATH = (
    Path(__file__).resolve().parent
    / "fixtures"
    / "fact_intake"
    / "protected_chat_input_v1.json"
)
...
def test_chat_import_builds_protected_envelope_without_raw_text_in_report() -> None:
    ...
    assert (
        normalized["entries"][0]["envelope_summary"]
        == "Chat message from Me at 2026-03-18T08:45:00Z"
    )
    assert report["version"] == "protected.disclosure.envelope.v1"
    assert (
        "I believe the procurement direction may have been improper." not in serialized
    )
    assert "Keep provisional until documentary support is assembled." not in serialized


def test_chat_import_script_writes_normalized_input_and_report(
    tmp_path, capsys
) -> None:
...
def test_build_handoff_from_chat_artifact_returns_paths(tmp_path) -> None:
    payload = build_handoff_from_chat_artifact(
        _PERSONAL_FIXTURE_PATH, tmp_path / "artifact"
    )
