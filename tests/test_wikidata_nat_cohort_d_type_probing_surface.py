...
def test_cohort_d_type_probing_surface_fixture_is_fail_closed_and_non_executing() -> (
    None
):
    ...
    assert all(
        row["cohort_flags"] == ["missing_instance_of_typing_deficit"]
...
def test_cohort_d_type_probing_surface_marks_missing_packet_refs_incomplete() -> None:
    ...
    one_packet = _load_fixture(
        "wikidata_nat_review_packet_Q738421_sidecar_20260402.json"
    )
