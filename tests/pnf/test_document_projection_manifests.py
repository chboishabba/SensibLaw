...
def test_document_projection_manifest_is_partition_independent() -> None:
    layer = {
        "token_annotations": [
            {"token_index": 0, "annotation_type": "token", "value": "Ada"}
        ],
        "span_annotations": [
            {
                "span_ref": "span:ada",
    ...
    parts = [
        build_partition_manifest(
            ...
            source_sha256="b" * 64,
            build_key_sha256="c" * 64,
            parser_contract_ref="parser:1",
            reducer_contract_ref="reducer:1",
    ...
    manifest = document_projection_join(
        partitions=parts, logical_layers=(logical,), canonical_length=10
    )

    assert (
        manifest.graph_ref
        == AnnotationGraph.from_layer_refs((logical.layer_ref,)).graph_ref
    )
