...
def test_fact_intake_example_validates() -> None:
    schema = yaml.safe_load(
        Path("schemas/fact.intake.bundle.v1.schema.yaml").read_text(encoding="utf-8")
    )
    payload = json.loads(
        Path("examples/fact_intake_bundle_minimal.json").read_text(encoding="utf-8")
    )
...
def test_build_fact_intake_payload_from_text_units_is_deterministic() -> None:
    ...
    payload_a = build_fact_intake_payload_from_text_units(
        units, source_label="mary_parity_demo", notes="demo"
    )
    payload_b = build_fact_intake_payload_from_text_units(
        units, source_label="mary_parity_demo", notes="demo"
    )
...
def test_persist_report_and_mary_projection_support_provenance_and_review_queue() -> (
    None
):
    ...
    payload = build_fact_intake_payload_from_text_units(
        units, source_label="mary_parity_demo"
    )
    ...
    projection = build_mary_fact_workflow_projection(
    ...
    acceptance = build_fact_review_acceptance_report(
        workbench, fixture_kind="synthetic"
    )
    ...
    assert (
        conn.execute(
            "SELECT COUNT(*) FROM entity_class_assertions WHERE run_id = ?",
            (payload["run"]["run_id"],),
        ).fetchone()[0]
        == persist_summary["semantic_assertion_count"]
    )
    ...
    assert report["facts"][0]["statement_ids"] == [
        payload["statements"][0]["statement_id"]
    ]
    ...
    assert projection["facts"][0]["provenance"]["source_ids"] == [
    ...
    assert (
        review_summary["review_queue"][0]["primary_contested_reason_text"]
        == "Later note gives a different date."
    )
    assert review_summary["review_queue"][0]["latest_review_status"] == "needs_followup"
    assert (
        review_summary["review_queue"][0]["latest_review_note"]
        == "Check primary clinical source."
    )
    assert (
        review_summary["review_queue"][0]["status_explanation"]["status_scope"]
        == "review"
    )
    assert (
        review_summary["review_queue"][0]["status_explanation"]["related_record_id"]
        == payload["fact_candidates"][0]["fact_id"]
    )
    ...
    assert (
        review_summary["review_queue"][0]["has_legal_procedural_observations"] is False
    )
    assert review_summary["review_queue"][0]["reason_codes"] == [
        "contested",
        "review_followup",
        "contradictory_chronology",
    ]
    assert review_summary["review_queue"][0]["chronology_bucket"] == "dated"
    assert review_summary["review_queue"][0]["event_ids"] == [
    ...
    assert (
        review_summary["review_queue"][1]["status_explanation"]["status_bucket"]
        == "review_source"
    )
    assert review_summary["review_queue"][1]["chronology_impacted"] is True
    assert review_summary["review_queue"][1]["reason_codes"] == [
        "unreviewed",
        "missing_actor",
    ...
    workbench_fact = next(
        ...
        if row["fact_id"] == review_summary["review_queue"][0]["fact_id"]
    )
    assert (
        workbench_fact["status_explanation"]
        == review_summary["review_queue"][0]["status_explanation"]
    )
    ...
    assert (
        operator_views["intake_triage"]["items"][0]["fact_id"]
        == payload["fact_candidates"][0]["fact_id"]
    )
    assert (
        operator_views["intake_triage"]["groups"]["contradictory_chronology"][0][
            "fact_id"
        ]
        == payload["fact_candidates"][0]["fact_id"]
    )
    assert (
        operator_views["intake_triage"]["control_plane"]["version"]
        == "follow.control.v1"
    )
    assert operator_views["intake_triage"]["queue"][0]["route_target"] in {
        "chronology_review",
        "actor_review",
        "procedural_review",
        "manual_review",
    ...
    assert (
        operator_views["intake_triage"]["queue"][0]["operator_readout"]["headline"]
        == operator_views["intake_triage"]["queue"][0]["title"]
    )
    assert (
        operator_views["intake_triage"]["queue"][0]["operator_readout"]["reason_line"]
        == operator_views["intake_triage"]["queue"][0]["description"]
    )
    assert any(
        row["label"] == "Status"
        for row in operator_views["intake_triage"]["queue"][0]["detail_rows"]
    )
    assert any(
    ...
    assert (
        workbench["operator_views"]["contested_items"]["queue"][0]["operator_readout"][
            "headline"
        ]
        == workbench["operator_views"]["contested_items"]["queue"][0]["title"]
    )
    assert (
        workbench["operator_views"]["contested_items"]["queue"][0]["operator_readout"][
            "reason_line"
        ]
        == workbench["operator_views"]["contested_items"]["queue"][0]["description"]
    )
    assert (
        operator_views["procedural_posture"]["items"][0]["fact_id"]
        == payload["fact_candidates"][1]["fact_id"]
    )
    assert (
        workbench["inspector_defaults"]["selected_fact_id"]
        == payload["fact_candidates"][0]["fact_id"]
    )
    assert workbench["operator_views"]["contested_items"]["summary"]["count"] == 1
    assert (
        workbench["operator_views"]["contested_items"]["control_plane"]["version"]
        == "follow.control.v1"
    )
    assert workbench["operator_views"]["contested_items"]["queue"][0][
        "resolution_status"
    ] in {"needs_followup", "reviewed", "open"}
    assert (
        workbench["reopen_navigation"]["current"]["workflow_kind"]
        == "transcript_semantic"
    )
    assert (
        workbench["reopen_navigation"]["query"]["workflow_run_id"] == "semantic:test:1"
    )
    assert (
        workbench["reopen_navigation"]["recent_sources"][0]["workflow_kind"]
        == "transcript_semantic"
    )
    assert (
        workbench["reopen_navigation"]["recent_sources"][0]["fact_run_id"]
        == payload["run"]["run_id"]
    )
    assert workbench["issue_filters"]["available_filters"] == [
        "all",
        "missing_actor",
        "contradictory_chronology",
        "procedural_significance",
    ...
    assert workbench["workflow_summary"]["stage"] in {
        "inspect",
        "decide",
        "record",
        "follow_up",
    }
    assert workbench["workflow_summary"]["recommended_view"] in {
        "intake_triage",
        "chronology_prep",
        "contested_items",
        "authority_follow",
    ...
    assert workbench["inspector_classification"]["facts"][
        payload["fact_candidates"][0]["fact_id"]
    ]["display_labels"] == ["unclassified"]
    assert (
        workbench["inspector_classification"]["facts"][
            payload["fact_candidates"][1]["fact_id"]
        ]["status_keys"]["party_assertion"]
        is True
    )
    ...
    assert workbench["zelph"]["rule_status"] in {
        "engine_ok",
        "engine_unavailable",
        "engine_error",
    }
    ...
    assert (
        workbench["facts"][0]["inspector_classification"]["dominant_label"]
        == "unclassified"
    )
    assert (
        workbench["facts"][1]["inspector_classification"]["dominant_label"]
        == "party_assertion"
    )
    ...
    assert (
        ...
        not in acceptance_by_id["SL-US-09"]["failed_check_ids"]
    )
    assert "reopen_navigation" not in acceptance_by_id["SL-US-10"]["failed_check_ids"]
    assert (
        list_fact_review_sources(conn, workflow_kind="transcript_semantic")[0][
            "latest_workflow_link"
        ]["fact_run_id"]
        == payload["run"]["run_id"]
    )
    assert (
        resolve_fact_run_link(
            conn, workflow_kind="transcript_semantic", workflow_run_id="semantic:test:1"
        )["fact_run_id"]
        == payload["run"]["run_id"]
    )
    assert (
    ...
    assert (
        find_latest_fact_workflow_link(conn, workflow_kind="transcript_semantic")[
            "fact_run_id"
        ]
        == payload["run"]["run_id"]
    )
...
def test_bounded_operator_views_expose_contract_keys() -> None:
    ...
    payload = build_fact_intake_payload_from_text_units(
        units, source_label="bounded_views_demo"
    )
    ...
    assert (
        operator_views["intake_triage"]["control_plane"]["source_family"]
        == "fact_review"
    )
    assert isinstance(operator_views["intake_triage"]["queue"], list)
    assert (
        operator_views["contested_items"]["control_plane"]["conjecture_kind"]
        == "contested_fact_item"
    )
...
def test_persist_fact_intake_payload_replaces_existing_run_rows() -> None:
    ...
    payload = build_fact_intake_payload_from_text_units(
        units, source_label="replace_demo"
    )
    persist_fact_intake_payload(conn, payload)
    ...
    observation_row_count = conn.execute(
        "SELECT COUNT(*) FROM fact_observations"
    ).fetchone()[0]
    event_row_count = conn.execute("SELECT COUNT(*) FROM event_candidates").fetchone()[
        0
    ]
...
def test_workbench_payload_applies_canonical_zelph_enrichment() -> None:
    ...
    payload = build_fact_intake_payload_from_text_units(
        units, source_label="zelph_demo"
    )
    payload["sources"][0]["provenance"] = {
        "source_signal_classes": ["wiki_article", "revision_history"]
    }
    payload["sources"][1]["provenance"] = {
        "source_signal_classes": ["legal_record", "strong_legal_source"]
    }
    ...
    wiki_fact = next(
        row
        for row in workbench["facts"]
        if row["fact_id"] == payload["fact_candidates"][0]["fact_id"]
    )
    legal_fact = next(
        row
        for row in workbench["facts"]
        if row["fact_id"] == payload["fact_candidates"][1]["fact_id"]
    )
...
def test_abstained_observation_is_preserved_but_not_used_for_event_assembly() -> None:
    ...
    assert {row["observation_status"] for row in report["observations"]} == {
        "captured",
        "abstained",
    }
...
def test_chat_archive_projection_mode_enriches_openrecall_like_text() -> None:
    ...
    payload = build_fact_intake_payload_from_text_units(
        units, source_label="openrecall_demo"
    )
    payload["sources"][0]["source_type"] = "openrecall_capture"
    payload["sources"][0].setdefault("provenance", {})["lexical_projection_mode"] = (
        "chat_archive"
    )
    ...
    assert (
        fact["status_explanation"] == summary["review_queue"][0]["status_explanation"]
    )
    assert {
        "self_correction_signal",
        "execution_handoff_signal",
        "uncertainty_preserved",
        "sequence_signal",
    } <= set(fact["signal_classes"])
    assert {
        "self_correction_signal",
        "execution_handoff_signal",
        "uncertainty_preserved",
        "sequence_signal",
    } <= set(fact["inferred_signal_classes"])
...
def test_transcript_handoff_projection_mode_preserves_handoff_and_uncertainty() -> None:
    ...
    fact = workbench["facts"][0]
    assert {
        "handoff_context_signal",
        "professional_handoff_signal",
        "uncertainty_preserved",
        "sequence_signal",
    } <= set(fact["signal_classes"])
    assert {
...
def test_au_legal_projection_mode_marks_appeal_and_stage_sensitive_language() -> None:
    ...
    fact = workbench["facts"][0]
    assert {"appeal_stage_signal", "procedural_outcome", "party_assertion"} <= set(
        fact["signal_classes"]
    )
    assert {"appeal_stage_signal", "procedural_outcome", "party_assertion"} <= set(
        fact["inferred_signal_classes"]
    )
...
def test_semantic_materialization_projects_source_relations_and_policies() -> None:
    ...
    payload = build_fact_intake_payload_from_text_units(
        units, source_label="semantic_relation_demo"
    )
    payload["fact_candidates"][0]["statement_ids"] = [
        row["statement_id"] for row in payload["statements"]
    ]
    payload["sources"][0]["provenance"] = {
...
def test_backfill_fact_semantics_script_rebuilds_semantic_rows() -> None:
    ...
    try:
        ...
        payload = build_fact_intake_payload_from_text_units(
            units, source_label="backfill_demo"
        )
        payload["sources"][0]["provenance"] = {
        ...
        conn.commit()
        conn.close()
        exit_code = backfill_fact_semantics_main(
            ["--db-path", str(db_path), "--run-id", run_id]
        )
        assert exit_code == 0
...
def test_semantic_refresh_runs_capture_status_transitions_and_timestamps() -> None:
    ...
    persist_fact_intake_payload(conn, payload)
    refreshes = list_semantic_refresh_runs(
        conn, run_id=payload["run"]["run_id"], limit=5
    )
    assert refreshes
    refresh = refreshes[0]
    ...
    conn.commit()
    refresh_mid = list_semantic_refresh_runs(
        conn, run_id=payload["run"]["run_id"], limit=1
    )[0]
    ...
    conn.commit()
    refresh_final = list_semantic_refresh_runs(
        conn, run_id=payload["run"]["run_id"], limit=1
    )[0]
...
def test_agent_feedback_payload_includes_policy_messages_and_counts() -> None:
    ...
    payload = build_fact_intake_payload_from_text_units(
        units, source_label="agent_feedback_demo"
    )
    ...
    payload["sources"][1].setdefault("provenance", {})["lexical_projection_mode"] = (
        "chat_archive"
    )
    ...
    assert (
        "Human review required before relying on this fact."
        in feedback["global_messages"]
    )
    assert (
        "Preserve the original source boundary in any summary or handoff."
        in feedback["global_messages"]
    )
