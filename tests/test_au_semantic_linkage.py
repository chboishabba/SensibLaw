...
def test_au_semantic_seed_import_and_matching(tmp_path: Path) -> None:
    ...
    ref_tuples = {
        (row["seed_id"], row["ref_kind"], row["canonical_ref"]) for row in stored_refs
    }
    assert (
    ...
    matched_seed_ids = {
        row["seed_id"] for row in report["per_seed"] if row["matched_count"] > 0
    }
    candidate_seed_ids = {
        row["seed_id"] for row in report["per_seed"] if row["candidate_count"] > 0
    }
    ...
    ev1_receipts = {
        ...
        for item in next(
            ...
            if match["seed_id"] == "au_semantic:mabo_native_title"
        )["receipts"]
    }
    assert (
        "jurisdiction_ref",
    ...
    ev6_receipts = {
        ...
        for item in next(
            ...
            if match["seed_id"] == "au_semantic:native_title_nsw_act_lane"
        )["receipts"]
    }
    ...
    ev7_receipts = {
        ...
        for item in next(
            ...
            if match["seed_id"] == "au_semantic:civil_liability_act_lane"
        )["receipts"]
    }
