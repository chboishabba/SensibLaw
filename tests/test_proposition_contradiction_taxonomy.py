...
def test_build_proposition_contradiction_taxonomy_exposes_expected_labels_and_exclusions() -> (
    None
):
    ...
    assert (
        "partial_overlap" in taxonomy["excluded_categories"]["ambiguity_incompleteness"]
    )
...
def test_validate_proposition_contradiction_label_accepts_supported_labels(
    label: str,
) -> None:
    assert validate_proposition_contradiction_label(label) == label


def test_validate_proposition_contradiction_label_rejects_review_pressure_and_unknown_labels() -> (
    None
):
