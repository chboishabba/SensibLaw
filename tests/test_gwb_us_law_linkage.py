...
def test_gwb_us_law_seed_import_and_matching(tmp_path: Path) -> None:
    ...
    assert any(
        match["seed_id"] == "gwb_us_law:clear_skies_2003" and match["matched"]
        for match in per_event["ev1"]["matches"]
    )
    assert any(
        match["seed_id"] == "gwb_us_law:stem_cell_research_enhancement_act"
        and match["matched"]
        for match in per_event["ev2"]["matches"]
    )
    assert any(
        match["seed_id"] == "gwb_us_law:nsa_surveillance_review"
        for match in per_event["ev3"]["matches"]
    )
    assert any(
        match["seed_id"] == "gwb_us_law:iraq_2002_authorization"
        and match["matched"]
    ...
    assert any(
        ...
        and any(
            receipt["kind"] == "provenance_cue_broad" for receipt in match["receipts"]
        )
        for match in per_event["ev5"]["matches"]
    )
    assert any(
        match["seed_id"] == "gwb_us_law:no_child_left_behind_act" and match["matched"]
        for match in per_event["ev6"]["matches"]
    )
    assert any(
        match["seed_id"]
        == "gwb_us_law:northwestern_hawaiian_islands_marine_national_monument"
