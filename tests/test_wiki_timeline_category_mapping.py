...
def test_timeline_extract_maps_categories_to_events(tmp_path: Path, capsys) -> None:
    ...
    exit_code = timeline_main(
        ["--snapshot", str(snapshot_path), "--out", str(out_path)]
    )
...
def test_candidates_extract_honors_include_categories(tmp_path: Path, capsys) -> None:
    ...
    candidates_main(
        ["--in", str(snapshot_path), "--out", str(out_path), "--include-categories"]
    )
    ...
    assert any(
        "1980 births" in r["title"] or "Category:1980 births" in r["title"]
        for r in payload_cats.get("rows", [])
    )
