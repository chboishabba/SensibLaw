...
def _build_au_report(tmp_path: Path) -> dict:
    ...
    seed_payload = json.loads(
        (Path("data/ontology/au_semantic_linkage_seed_v1.json")).read_text(
            encoding="utf-8"
        )
    )
    timeline_payload = {
...
def _build_gwb_report(tmp_path: Path) -> dict:
    ...
    seed_payload = json.loads(
        (Path("data/ontology/gwb_us_law_linkage_seed_v1.json")).read_text(
            encoding="utf-8"
        )
    )
    timeline_payload = {
...
def test_cross_system_phi_prototype_builds_from_real_promoted_reports(
    tmp_path: Path,
) -> None:
    ...
    assert all(
        row["graph_version"] == "sl.latent_promoted_graph.v1"
        for row in payload["latent_graphs"]
    )
    ...
    assert (
        payload["mismatch_report"]["workflow"]["workflow_id"]
        == "sl.phi_mismatch_review.v1"
    )
    ...
    partial_mapping = next(
        row for row in payload["mappings"] if row["status"] == "partial"
    )
    incompatible_mapping = next(
        row for row in payload["mappings"] if row["status"] == "incompatible"
    )
    ...
    assert partial_mapping["meta_validation"]["witness"]["authority_alignment"][
        "relation"
    ] in {"exact", "analogue"}
    assert partial_mapping["mapping_explanation"]["meta_summary"][
        "authority_relation"
    ] in {"exact", "analogue"}
    assert (
        partial_mapping["mapping_explanation"]["witness"]["type_alignment"]["relation"]
        == "exact"
    )
    assert partial_mapping["mapping_explanation"]["latent_graph_refs"][
    ...
    assert incompatible_mapping["mapping_explanation"]["meta_summary"][
        "constraint_status"
    ] in {"compatible", "conditional"}

    provenance_index = {
        row["provenance_ref"]: row for row in payload["provenance_index"]
    }
    for mapping in payload["mappings"]:
        for provenance_ref in mapping["provenance_refs"]:
            ...
            assert (
                provenance_row["source_char_end"] > provenance_row["source_char_start"]
            )
    ...
    for diagnostic in payload["mismatch_report"]["diagnostics"]:
        ...
        systems = {
            provenance_index[ref]["system_id"] for ref in diagnostic["provenance_refs"]
        }
    ...
    assert (
        ...
        in payload["mismatch_report"]["incompatible_mapping_ids"]
    )
    ...
    assert (
        "authority_relation_incompatible"
        in blocked_pair["meta_validation"]["violations"]
    )
    assert (
        blocked_pair["source_ref"]
        in payload["meta_validation_report"]["blocked_source_refs"]
    )
    assert (
        blocked_pair["meta_validation"]["witness"]["authority_alignment"]["relation"]
        == "incompatible"
    )


def test_cross_system_phi_prototype_uses_default_meta_contract_shape() -> None:
    contract = build_default_phi_meta_contract(
        left_system="au_hca", right_system="us_exec_judicial"
    )
    assert contract["version"] == "sl.cross_system_phi_meta.v1"
    assert contract["thresholds"]["min_total_score"] == 0.72


def test_cross_system_phi_minimal_example_still_validates_under_extended_schema() -> (
    None
):
    payload = json.loads(
        Path("examples/cross_system_phi_minimal.json").read_text(encoding="utf-8")
    )
    jsonschema.validate(payload, _load_schema())
