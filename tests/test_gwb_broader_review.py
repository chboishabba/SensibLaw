...
def test_build_gwb_broader_review(tmp_path: Path) -> None:
    ...
    assert any(
        row["role"] == "legal_linkage_graph"
        for row in payload["compiler_contract"]["derived_products"]
    )
    ...
    assert all(
        row["family_id"] == "gwb_broader_review"
        for row in payload["review_claim_records"]
    )
    assert all(
        row["state"] == "review_claim" for row in payload["review_claim_records"]
    )
    assert all(
        row["state_basis"] == "source_review_row"
        for row in payload["review_claim_records"]
    )
    assert all(
        row["evidence_status"] == "review_only"
        for row in payload["review_claim_records"]
    )
    assert all(
        row["review_route"]["actionability"] == "must_review"
        for row in payload["review_claim_records"]
    )
    relation_rows = [
        row for row in payload["review_claim_records"] if "proposition_relation" in row
    ]
    assert relation_rows
    assert (
        relation_rows[0]["target_proposition_identity"]["identity_basis"]["basis_kind"]
        == "seed_id"
    )
    assert (
        relation_rows[0]["target_proposition_identity"]["provenance"]["source_kind"]
        == "review_item_target"
    )
    assert relation_rows[0]["review_candidate"]["candidate_kind"] == "review_source_row"
    assert (
        relation_rows[0]["review_candidate"]["selection_basis"]["basis_kind"]
        == "source_review_row"
    )
    assert (
        relation_rows[0]["review_candidate"]["selection_basis"]["review_status"]
        == "missing_review"
    )
    assert (
        relation_rows[0]["review_candidate"]["anchor_refs"]["source_row_id"]
        == relation_rows[0]["claim_id"]
    )
    assert (
    ...
    assert (
        relation_rows[0]["proposition_relation"]["target_proposition_id"]
        == relation_rows[0]["target_proposition_identity"]["proposition_id"]
    )
    assert relation_rows[0]["review_text"]["text_role"] == "review_source_text"
    assert (
        relation_rows[0]["review_text"]["source_kind"]
        == relation_rows[0]["provenance"]["source_kind"]
    )
    assert any(
        "proposition_relation" not in row for row in payload["review_claim_records"]
    )
    ...
    assert normalized_artifact["unresolved_pressure_status"] in {
        "none",
        "hold",
        "abstain",
    }
    ...
    assert (
        ...
        == payload["legal_follow_graph"]["summary"]["node_count"]
    )
    assert (
        graph_diagnostics["metrics"]["edge_count"]
        == payload["legal_follow_graph"]["summary"]["edge_count"]
    )
    ...
    assert payload["workflow_summary"]["stage"] in {
        "decide",
        "follow_up",
        "record",
        "archive",
    ...
    assert (
        payload["workflow_summary"]["counts"]["debate_edge_count"]
        == summary["debate_edge_count"]
    )
    assert summary["debate_edge_count"] > 0
    assert payload["workflow_summary"]["promotion_gate"]["decision"] in {
        "promote",
        "audit",
        "abstain",
    }
    ...
    assert (
        summary["provisional_review_bundle_count"]
        >= summary["related_review_cluster_count"]
    )
    ...
    assert (
        normalized["candidate_signal_count"]
        == normalized["provisional_queue_row_count"]
    )
    ...
    assert (
        normalized["provisional_row_density"] == normalized["candidate_signal_density"]
    )
    assert normalized["provisional_bundle_density"] >= 0.9

    assert any(
        row["source_kind"] == "seed_family_support"
        for row in payload["source_review_rows"]
    )
    assert any(
        row["source_kind"] == "merged_promoted_relation"
        for row in payload["source_review_rows"]
    )
    assert any(
        row["source_kind"] == "source_family_summary"
        for row in payload["source_review_rows"]
    )
    merged_relation_row = next(
        row
        for row in payload["source_review_rows"]
        if row["source_kind"] == "merged_promoted_relation"
        and (row["source_paths"] or row["source_urls"])
    )
    assert merged_relation_row["event_ids"]
    assert merged_relation_row["metadata"]["promoted_relation_id"]
    assert merged_relation_row["metadata"]["event_lineage_depth"] in {
        "complete",
        "partial",
    }
    ...
    assert (
        payload["legal_follow_graph"]["summary"]["seed_lane_count"]
        >= summary["distinct_seed_lane_count"]
    )
    assert (
        payload["legal_follow_graph"]["summary"]["seed_lane_count"]
    ...
    assert (
        payload["legal_follow_graph"]["summary"]["source_row_count"]
        == summary["source_row_count"]
    )
    assert (
        payload["legal_follow_graph"]["summary"]["source_row_node_count"]
        <= summary["source_row_count"]
    )
    assert payload["legal_follow_graph"]["summary"]["source_family_count"] >= 1
    assert (
        payload["legal_follow_graph"]["summary"]["source_kind_counts"].get(
            "source_family_summary", 0
        )
        >= 1
    )
    assert isinstance(
        payload["legal_follow_graph"]["summary"]["source_family_label_counts"], dict
    )
    assert isinstance(
        payload["legal_follow_graph"]["summary"]["linkage_kind_counts"], dict
    )
    assert isinstance(
        payload["legal_follow_graph"]["summary"]["review_status_label_counts"], dict
    )
    assert isinstance(
        payload["legal_follow_graph"]["summary"]["support_kind_label_counts"], dict
    )
    assert isinstance(
        payload["legal_follow_graph"]["summary"]["followed_source_cite_class_counts"],
        dict,
    )
    ...
    assert (
        payload["operator_views"]["legal_follow_graph"]["control_plane"]["version"]
        == "follow.control.v1"
    )
    assert (
        payload["operator_views"]["legal_follow_graph"]["control_plane"][
            "source_family"
        ]
        == "gwb_legal_follow"
    )
    assert isinstance(
        payload["operator_views"]["legal_follow_graph"]["summary"][
            "route_target_counts"
        ],
        dict,
    )
    ...
    assert payload["operator_views"]["legal_follow_graph"]["summary"][
    ...
    assert isinstance(
        payload["operator_views"]["legal_follow_graph"]["highlight_nodes"], list
    )
    assert isinstance(
        payload["operator_views"]["legal_follow_graph"]["sample_edges"], list
    )
    if payload["operator_views"]["legal_follow_graph"]["queue"]:
        assert (
            payload["operator_views"]["legal_follow_graph"]["queue"][0]["priority_rank"]
            == 1
        )
    ...
    parliamentary_rows = [
        row
        for row in payload["source_review_rows"]
        if row.get("source_kind") == "parliamentary_statement"
    ]
    assert len(parliamentary_rows) == 2
    assert all(
        row["metadata"]["source_unit_id"].startswith("sourceunit:parliamentary")
    ...
    assert all(
        row["metadata"].get("ready_for_follow") is True for row in parliamentary_rows
    )
    assert any(
        anchor.get("anchor_kind") == "clause_label"
        for row in parliamentary_rows
    ...
    receipt_rows = [
        row
        for row in payload["source_review_rows"]
        if row.get("source_kind") == "uk_legislation_receipt"
    ]
    assert receipt_rows
    anchor_kinds = {
        anchor["anchor_kind"] for anchor in receipt_rows[0]["candidate_anchors"]
    }
...
def test_broader_review_item_rows_split_multi_family_seed_into_multiple_candidates() -> (
    None
):
...
def test_build_gwb_broader_review_emits_revision_stability_for_explicit_baseline_pair(
    tmp_path: Path,
) -> None:
    baseline_result = build_gwb_broader_review(tmp_path / "baseline-out")
    baseline_payload = json.loads(
        Path(baseline_result["artifact_path"]).read_text(encoding="utf-8")
    )
    baseline_graph_diagnostics = baseline_payload["suite_normalized_artifact"][
        "graph_diagnostics"
    ]
    ...
    assert (
        revision_stability["comparison_scope"]["comparison_basis"]
        == "explicit_graph_diagnostics_pair"
    )
    assert (
        revision_stability["baseline_ref"]["source_artifact_id"]
        == baseline_graph_diagnostics["scope"]["source_artifact_id"]
    )
    assert (
        revision_stability["candidate_ref"]["source_artifact_id"]
        == graph_diagnostics["scope"]["source_artifact_id"]
    )
    ...
    assert (
        payload["reasoner_input_artifact"]["normalized_artifact"]["graph_diagnostics"][
            "revision_stability"
        ]
        == revision_stability
    )
...
def test_gwb_broader_review_world_model_report_rebinds_legal_follow_queue(
    tmp_path: Path,
) -> None:
    result = build_gwb_broader_review(tmp_path / "out")
    payload = json.loads(Path(result["artifact_path"]).read_text(encoding="utf-8"))
    ...
    assert (
        report["summary"]["queue_count"]
        == payload["operator_views"]["legal_follow_graph"]["summary"]["queue_count"]
    )
    ...
    assert any(
        any(
            ref.get("event_ids")
            for ref in claim["nat_claim"]["qualifiers"]["source_refs"]
        )
        for claim in report["claims"]
    )
    assert any(
        any(
            ref.get("cross_source_braid_depth")
            for ref in claim["nat_claim"]["qualifiers"]["source_refs"]
        )
        for claim in report["claims"]
    )
...
def test_gwb_broader_review_linkage_case_uses_projected_input_before_receipt(
    tmp_path: Path,
) -> None:
    result = build_gwb_broader_review(tmp_path / "out")
    payload = json.loads(Path(result["artifact_path"]).read_text(encoding="utf-8"))
...
def test_gwb_broader_review_lane_receipt_attaches_at_wrapper_boundary(
    tmp_path: Path,
) -> None:
    result = build_gwb_broader_review(tmp_path / "out")
    payload = json.loads(Path(result["artifact_path"]).read_text(encoding="utf-8"))
    ...
    assert (
        receipt["diagnostics"]["visibility_requirements"]["queue_review_depth"][
            "present"
        ]
        is True
    )
    assert (
        receipt["diagnostics"]["visibility_requirements"]["event_lineage_depth"][
            "present"
        ]
        is True
    )
    assert (
        receipt["diagnostics"]["visibility_requirements"]["cross_source_braid_depth"][
            "present"
        ]
        is True
    )
    assert (
        "complete"
        in receipt["diagnostics"]["visibility_requirements"]["event_lineage_depth"][
            "values"
        ]
    )
