...
def test_resolve_sqlite_db_path_prefers_explicit_then_env(
    ...
    assert (
        resolve_sqlite_db_path(str(explicit), env_vars=("ITIR_DB_PATH",))
        == explicit.resolve()
    )
    assert (
        resolve_sqlite_db_path(None, env_vars=("ITIR_DB_PATH",)) == env_path.resolve()
    )
