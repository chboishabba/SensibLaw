...
def test_cohort_d_operator_review_surface_becomes_incomplete_when_probe_surface_has_unresolved_refs() -> (
    None
):
    probe = _load_fixture("wikidata_nat_cohort_d_type_probing_surface_20260402.json")
    probe["unresolved_packet_refs"] = [
        {"review_entity_qid": "Q1785637", "reason": "missing_packet_payload"}
    ]

    payload = build_wikidata_nat_cohort_d_operator_review_surface(
        type_probing_surface=probe
    )
