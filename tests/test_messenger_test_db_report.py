...
from SensibLaw.scripts.ingest_messenger_sample_to_itir_test_db import (
    _classify_row,
    _split_sender_message_contamination,
)
...
def test_messenger_row_classifier_filters_noise_deterministically():
    assert (
        _classify_row(
            _FakeRow(
                sender="Facebook Marketplace Assistant",
                message="Your item has been listed",
                conversation="x",
                time_sent="2026-03-07T00:00:00Z",
            )
        )
    ...
    assert (
        _classify_row(
            _FakeRow(
                sender="Alice",
                message="Call started: 3 participants",
                conversation="x",
                time_sent="2026-03-07T00:00:00Z",
            )
        )
    ...
    assert (
        _classify_row(
            _FakeRow(
                sender="Alice",
                message="Here is the actual message body.",
                conversation="x",
                time_sent="2026-03-07T00:00:00Z",
            )
        )
...
def test_messenger_sender_message_contamination_is_split_deterministically():
    sender, message = _split_sender_message_contamination(
        "FacebookWe didn’t remove the ad", "Thanks again for your report."
    )
    assert sender == "Facebook"
    assert message == "We didn’t remove the ad Thanks again for your report."

    sender, message = _split_sender_message_contamination(
        "Chboi ShabbaWe received your report", "Thank you."
    )
...
def test_messenger_report_script_outputs_filter_counts_and_structure(tmp_path: Path):
        ...
        conn.executemany(
            ...
            [
                (
                    ...
                    "Alice",
                    "Check ./SensibLaw/tests/test_lexeme_layer.py",
                ),
                (
                    "messenger-test:fixture",
                    2,
    ...
    script = (
        ...
        / "scripts"
        / "report_messenger_test_tokenizer_stats.py"
    )
    ...
    top_atoms = {
        (row["norm_text"], row["kind"]) for row in payload["top_structural_atoms"]
    }
