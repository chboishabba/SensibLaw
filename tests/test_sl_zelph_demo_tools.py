...
def test_compile_db_smoke(tmp_path):
    ...
    conn.execute(
        "INSERT INTO rule_atoms VALUES ('d1', 's1', 'party1', 'role1', 'modality.must', 'action1', 'scope1')"
    )
    conn.commit()
    conn.close()
...
def test_wikidata_extract_smoke(tmp_path):
    ...
    result = subprocess.run(
        [PYTHON_EXE, str(script_path), str(input_txt)], capture_output=True, text=True
    )
    ...
    assert (
        data["wikidata_enrichment"][0]["properties"][0]["value_label"] == "supermarket"
    )
...
def test_lex_to_zelph_smoke(tmp_path):
    ...
    wiki_json.write_text(
        json.dumps(
            {
                "title": "Slip and fall",
                "rows": [{"revid": "123", "user": "Alice", "comment": "Fixed typo"}],
            }
        )
    )
...
def test_compile_ontology_smoke(tmp_path):
    ...
    assert (
        "Wikidata Bridge Bodies" in content
        or "AU Semantic Linkage" in content
        or "Wrong Type Catalog" in content
    )
...
def test_compile_db_extended(tmp_path):
    ...
    conn.execute(
        "INSERT INTO rule_atoms VALUES ('d2', 's2', '\"Party A\"', 'Role & B', 'modality.must', 'Action (X)', 'Scope [Y]')"
    )
    # Test case 2: Missing optional fields
    conn.execute(
        "INSERT INTO rule_atoms VALUES ('d3', 's3', 'Party B', NULL, 'modality.may', 'Action Z', NULL)"
    )
    conn.commit()
    conn.close()
...
def test_wikidata_extract_deep(tmp_path):
    ...
    enrichment = next(
        e for e in data["wikidata_enrichment"] if e["entity"] == "woolworths"
    )
    properties = {
        p["property_label"]: p["value_label"] for p in enrichment["properties"]
    }
...
def test_sl_extract_facts_content(tmp_path):
    ...
    facts = data["facts"]
    assert any(
        f["type"] == "event_occurred" and f["event_id"] == "slip_event" for f in facts
    )
    assert any(
        f["type"] == "condition" and f["condition_id"] == "wet_floor" for f in facts
    )
