...
def test_revision_pack_summary_builds_triage_and_human_summary() -> None:
    payload = build_run_summary(
        schema_version="wiki_revision_pack_state_v0_6",
        pack_id="pack",
        run_id="run:1",
                ...
                "pair_reports": [
                    {
                        ...
                        "top_changed_sections": [
                            {"section": "History", "touched_bytes": 1200}
                        ],
                    }
                ],
    ...
    assert (
        payload["pack_triage"]["top_changed_articles"][0]["article_id"] == "article_1"
    )
    assert (
        payload["pack_triage"]["top_high_severity_pairs"][0]["pair_kind"]
        == "largest_delta_in_window"
    )
    assert payload["pack_triage"]["top_sections_changed"][0]["section"] == "History"
    assert "report_path" not in payload["pack_triage"]["top_changed_articles"][0]
    assert (
        "pair_report_path" not in payload["pack_triage"]["top_high_severity_pairs"][0]
    )
    assert (
        "contested_graph_path" not in payload["pack_triage"]["top_contested_graphs"][0]
    )
