...
def test_build_nat_cohort_b_operator_control_summary_matches_pinned_fixture() -> None:
    index1 = _load_fixture(
        "wikidata_nat_cohort_b_operator_evidence_index_20260402.json"
    )
    index2 = _load_fixture(
        "wikidata_nat_cohort_b_operator_evidence_index_case2_20260402.json"
    )
    expected = _load_fixture(
        "wikidata_nat_cohort_b_operator_control_summary_20260402.json"
    )
...
def test_build_nat_cohort_b_operator_control_summary_holds_on_insufficient_ready_indexes() -> (
    None
):
    index2 = _load_fixture(
        "wikidata_nat_cohort_b_operator_evidence_index_case2_20260402.json"
...
def test_build_nat_cohort_b_operator_control_summary_holds_on_validation_error() -> (
    None
):
    invalid = {"cohort_id": "cohort_x", "index_status": "review_index_ready"}
    payload = build_nat_cohort_b_operator_control_summary(
        [invalid], min_ready_indexes=1
    )
