...
def test_sample_debates_reference_instruments():
    ...
    assert (
        "law:uk:climate_act"
        in debates["debate:uk:commons:2023:climate"].referenced_instruments
    )
    assert "defense_policy" in debates["debate:aus:senate:2022:defense"].influence_tags
    assert (
        "brexit_environment" in debates["debate:uk:commons:2023:climate"].influence_tags
    )
...
def test_export_records_serialized():
    ...
    assert (
        ...
        in exported["debate:uk:commons:2023:climate"]["edges"]
    )
