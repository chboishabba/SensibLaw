...
def test_gwb_relation_semantic_basis_requires_subject_object_and_predicate_receipts() -> (
    None
):
    structural = derive_relation_semantic_basis(
        receipts=[
            {"kind": "subject", "value": "George W. Bush"},
            {"kind": "verb", "value": "signed"},
            {
                "kind": "object_legal_ref",
                "value": "legal_ref:military_commissions_act_of_2006",
            },
        ],
        subject={"canonical_key": "actor:george_w_bush"},
        object_={"canonical_key": "legal_ref:military_commissions_act_of_2006"},
    )
    mixed = derive_relation_semantic_basis(
        receipts=[
            {"kind": "subject", "value": "George W. Bush"},
            {"kind": "verb", "value": "signed"},
        ],
...
def test_gwb_semantic_pipeline_promotes_actor_and_relation_rows(tmp_path: Path) -> None:
    ...
    seed_path = (
        ...
        / "data"
        / "ontology"
        / "gwb_us_law_linkage_seed_v1.json"
    )
    ...
    timeline_payload = {
        ...
        "parser": {"name": "fixture"},
        "source_timeline": {
            "path": str(tmp_path / "wiki_timeline_gwb.json"),
            "snapshot": None,
        },
        "events": [
            {
    ...
    promoted = {
        (
            row["predicate_key"],
            row["subject"]["canonical_key"],
            row["object"]["canonical_key"],
        )
    ...
    assert (
        "signed",
        "actor:george_w_bush",
        "legal_ref:military_commissions_act_of_2006",
    ) in promoted
    assert (
        "vetoed",
        "actor:george_w_bush",
    ...
    signed_row = next(
        row for row in report["promoted_relations"] if row["predicate_key"] == "signed"
    )
    signed_receipts = {
        (receipt["kind"], receipt["value"]) for receipt in signed_row["receipts"]
    }
    assert ("rule_type", "executive_action") in signed_receipts
    assert ("promotion_status", "promoted") in signed_receipts
    assert (
        signed_row["semantic_candidate"]["schema_version"]
    ...
    assert report["summary"]["semantic_basis_counts"]["structural"] == len(
    ...
    assert any(
        anchor["source"] in {"mention", "receipt", "label_fallback"}
        for anchor in signed_debug["anchors"]
    )
    assert all(
        isinstance(anchor["charStart"], int) and isinstance(anchor["charEnd"], int)
        for anchor in signed_debug["anchors"]
    )
    ...
    signed_event = next(
        event
        for event in report["text_debug"]["events"]
        if any(rel["predicateKey"] == "signed" for rel in event["relations"])
    )
...
def test_gwb_semantic_report_remains_receipt_free(tmp_path: Path) -> None:
    db_path = tmp_path / "itir.sqlite"
    seed_path = (
        Path(__file__).resolve().parents[1]
        / "data"
        ...
        "events": [
            {
                ...
                "section": "Confirmations",
                "text": "John Roberts was confirmed by the Senate on September 29, 2005.",
                "source_id": "https://en.wikipedia.org/wiki/John_Roberts",
            },
            {
                "event_id": "ev2",
                "anchor": {"year": 2006, "text": "October 17, 2006"},
...
def test_gwb_semantic_lane_wrapper_attaches_narrative_linkage_receipt(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "itir.sqlite"
    seed_path = (
    ...
    assert (
        receipt["contract"]["contract_id"] == GWB_NARRATIVE_TIMELINE_LINKAGE_CONTRACT_ID
    )
...
def test_gwb_narrative_world_model_uses_generic_adapter_stack(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        world_model = build_world_model(
            conn, profile="narrative_timeline", run_id=result["run_id"]
        )
...
def test_gwb_narrative_timeline_linkage_case_projects_composed_adapter_geometry(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "itir.sqlite"
    seed_path = (
    ...
    assert audited["anchor_to_tranche_reachability"]["all_reachable"] is True
    assert audited["candidate_vs_promoted_visibility"] is True
    assert audited["visibility_requirements"]["cross_source_provenance_visibility"][
        "values"
    ] == ["complete"]
...
def test_attach_gwb_narrative_timeline_linkage_receipt_rejects_raw_semantic_report(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "itir.sqlite"
    seed_path = (
    ...
    with pytest.raises(ValueError, match="project_linkage_case"):
        attach_receipt(report, profile="narrative_timeline")


def test_attach_gwb_narrative_timeline_linkage_receipt_attaches_to_projected_report(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "itir.sqlite"
    seed_path = (
    ...
    assert (
        wrapped["linkage_depth_receipt"]["contract"]["contract_id"]
        == GWB_NARRATIVE_TIMELINE_LINKAGE_CONTRACT_ID
    )
    assert wrapped["linkage_depth_receipt"]["case_id"] == "gwb_narrative_timeline"


def test_gwb_semantic_pipeline_stays_stable_with_au_linkage_imported(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "itir.sqlite"
    gwb_seed_path = (
    ...
    assert not any(
        row["entity"]["canonical_key"].startswith("actor:high_court_of_australia")
        for row in report["per_entity"]
    )
    assert all(
...
def test_gwb_semantic_pipeline_uses_root_actor_for_first_person_stem_cell_veto(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "itir.sqlite"
    seed_path = (
...
def test_gwb_semantic_schema_populates_shared_actor_aliases_and_role_vocab() -> None:
    ...
    role_vocab = {
        ...
        for row in conn.execute(
            "SELECT role_key, display_label FROM event_role_vocab"
        ).fetchall()
    }
    ...
    rule_types = {
        ...
        for row in conn.execute(
            "SELECT rule_type_key, output_kind FROM semantic_rule_types"
        ).fetchall()
    }
