...
class FakeZelphBackend:
    ...
    facts: dict[tuple[int, int], tuple[WikidataPropertyFact, ...]] = field(
...
class FakeLiveTransport:
    candidates: dict[str, tuple[WikidataSearchCandidate, ...]] = field(
        default_factory=dict
    )
    facts: dict[tuple[int, int], tuple[WikidataPropertyFact, ...]] = field(
        default_factory=dict
...
def test_snapshot_candidate_hit_does_not_touch_live_transport() -> None:
    ...
    assert [row.qid for row in result.candidates_by_label["Springfield"]] == [
        180672,
        28515,
    ]
    ...
    assert (
        result.candidates_by_label["Springfield"][0].source_epoch == JUNE_SNAPSHOT_EPOCH
    )
...
def test_resident_snapshot_can_report_zero_external_acquisition_calls() -> None:
    backend = FakeZelphBackend(
        labels={"Springfield": (180672,)}, search_acquisition_calls=0
    )
...
def test_june_snapshot_is_skipped_when_request_requires_july_freshness() -> None:
    ...
    result = transport.fetch_properties(
        (key,), minimum_source_epoch=JULY_FRESHNESS_FLOOR
    )
...
def test_freshness_sensitive_policy_can_force_live_recheck_even_when_snapshot_is_fresh_enough() -> (
    None
):
    key = (180672, 17)
    snapshot_fact = WikidataPropertyFact(
