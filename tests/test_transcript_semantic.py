...
def test_transcript_semantic_pipeline_persists_speakers_and_candidate_reply_relations() -> (
    None
):
    ...
    replied_row = next(
        row
        for row in report["candidate_only_relations"]
        if row["predicate_key"] == "replied_to"
    )
    assert replied_row["confidence_tier"] == "low"
    replied_receipts = {
        (receipt["kind"], receipt["value"]) for receipt in replied_row["receipts"]
    }
    assert ("rule_type", "conversational_relation") in replied_receipts
    assert ("promotion_status", "candidate") in replied_receipts
    assert (
        replied_row["semantic_candidate"]["schema_version"]
    ...
    assert (
        replied_row["canonical_promotion_reason"] == "relation_candidate_not_promoted"
    )
...
def test_transcript_semantic_pipeline_abstains_on_timing_only_and_role_only_sources() -> (
    None
):
    conn = sqlite3.connect(":memory:")
    conn.row_factory = sqlite3.Row
    ...
    units = [
        TextUnit(
            "u1", "src1", "transcript_file", "[00:00:00,030 -> 00:00:21,970] Thanks."
        ),
        TextUnit("u2", "src1", "chat_test_db", "User: run the tests"),
    ]
    result = run_transcript_semantic_pipeline(
        conn, units, run_id="transcript-fixture-v2"
    )
    report = build_transcript_semantic_report(
        conn, run_id=result["run_id"], units=units
    )
    ...
    assert (
        "[00:00:00,030 -> 00:00:21,970] Thanks.",
        "transcript_timing_only_v1",
    ) in unresolved
...
def test_transcript_semantic_pipeline_extracts_general_freeform_entities_without_legalizing_text() -> (
    None
):
    conn = sqlite3.connect(":memory:")
    conn.row_factory = sqlite3.Row
    ...
    result = run_transcript_semantic_pipeline(
        conn, units, run_id="transcript-fixture-v3"
    )
    report = build_transcript_semantic_report(
        conn, run_id=result["run_id"], units=units
    )
    ...
    felt_state_row = next(
        row
        for row in report["candidate_only_relations"]
        if row["predicate_key"] == "felt_state"
    )
...
def test_transcript_relation_semantic_basis_marks_partial_receipt_spines_as_mixed() -> (
    None
):
    assert (
...
def test_transcript_semantic_pipeline_abstains_on_obvious_titlecase_noise() -> None:
    ...
    result = run_transcript_semantic_pipeline(
        conn, units, run_id="transcript-fixture-v4"
    )
    report = build_transcript_semantic_report(
        conn, run_id=result["run_id"], units=units
    )
...
def test_transcript_semantic_pipeline_keeps_multi_token_names_and_place_context() -> (
    None
):
    conn = sqlite3.connect(":memory:")
    conn.row_factory = sqlite3.Row
    ...
    result = run_transcript_semantic_pipeline(
        conn, units, run_id="transcript-fixture-v5"
    )
    report = build_transcript_semantic_report(
        conn, run_id=result["run_id"], units=units
    )
...
def test_transcript_semantic_pipeline_extracts_explicit_social_relations_as_candidate_only() -> (
    None
):
    conn = sqlite3.connect(":memory:")
    conn.row_factory = sqlite3.Row
    ...
    result = run_transcript_semantic_pipeline(
        conn, units, run_id="transcript-fixture-v6"
    )
    report = build_transcript_semantic_report(
        conn, run_id=result["run_id"], units=units
    )
    ...
    assert result["promoted_relation_count"] == 0
    assert all(
        row["semantic_candidate"]["candidate_kind"] == "semantic_relation"
        for row in report["relation_candidates"]
    )

    sibling_row = next(
        row
        for row in report["candidate_only_relations"]
        if row["predicate_key"] == "sibling_of"
    )
    sibling_receipts = {
        (receipt["kind"], receipt["value"]) for receipt in sibling_row["receipts"]
    }
    ...
    guardian_row = next(
        row
        for row in report["candidate_only_relations"]
        if row["predicate_key"] == "guardian_of"
    )
    care_row = next(
        row
        for row in report["candidate_only_relations"]
        if row["predicate_key"] == "caregiver_of"
    )
    guardian_receipts = {
        (receipt["kind"], receipt["value"]) for receipt in guardian_row["receipts"]
    }
    care_receipts = {
        (receipt["kind"], receipt["value"]) for receipt in care_row["receipts"]
    }
    assert ("cue_surface", "guardian") in guardian_receipts
    assert ("cue_surface", "cared_for") in care_receipts
    assert any(
        value in {"looks_after", "cared_for"}
        for kind, value in care_receipts
        if kind == "cue_surface"
    )
    social_debug = next(
        relation
...
def test_transcript_semantic_report_persists_mission_observer_and_review_submissions_in_db() -> (
    None
):
    conn = sqlite3.connect(":memory:")
    conn.row_factory = sqlite3.Row
    ...
    result = run_transcript_semantic_pipeline(
        conn, units, run_id="transcript-db-review-v1"
    )
    report = build_transcript_semantic_report(
        conn, run_id=result["run_id"], units=units
    )

    mission_run = conn.execute(
        "SELECT run_id, source FROM mission_runs WHERE run_id = ?", (result["run_id"],)
    ).fetchone()
    ...
    assert (
        conn.execute(
            "SELECT COUNT(*) FROM mission_nodes WHERE run_id = ?", (result["run_id"],)
        ).fetchone()[0]
        >= 1
    )
    assert (
        conn.execute(
            "SELECT COUNT(*) FROM mission_observer_overlays WHERE run_id = ?",
            (result["run_id"],),
        ).fetchone()[0]
        >= 1
    )
    ...
    rows = list_semantic_review_submissions(
        conn, source="transcript", run_id=str(result["run_id"])
    )
    assert len(rows) == 1
...
def test_mission_lens_report_builds_actual_vs_should_artifact(tmp_path) -> None:
    ...
    with sqlite3.connect(itir_db) as conn:
        ...
        result = run_transcript_semantic_pipeline(
            conn, units, run_id="mission-lens-fixture-v1"
        )
    ...
    upsert_dashboard_payload(
        ...
        payload={
            ...
            "chat_threads": [
                {
                    "thread_id": "chat-a",
                    "title": "Notification routing feature",
                    "message_count": 2,
                    "source_ids": ["chat-a"],
                }
            ],
    ...
    report = build_mission_lens_report(
        ...
        sb_db_path=sb_db,
        date="2026-03-08",
        run_id="mission-lens-fixture-v1",
    )
...
def test_mission_lens_report_prefers_reviewed_actual_mapping_over_lexical_fallback(
    tmp_path,
) -> None:
    from scripts.mission_lens import build_mission_lens_report
    from sb.dashboard_store_sqlite import DashboardKey, upsert_dashboard_payload
    ...
    with sqlite3.connect(itir_db) as conn:
        ...
        result = run_transcript_semantic_pipeline(
            conn, units, run_id="mission-lens-fixture-v2"
        )
        build_transcript_semantic_report(conn, run_id=result["run_id"], units=units)
...
def test_mission_lens_unlinked_reviewed_mapping_suppresses_lexical_fallback(
    tmp_path,
) -> None:
    from scripts.mission_lens import build_mission_lens_report
    from sb.dashboard_store_sqlite import DashboardKey, upsert_dashboard_payload
    ...
    with sqlite3.connect(itir_db) as conn:
        ...
        result = run_transcript_semantic_pipeline(
            conn, units, run_id="mission-lens-fixture-v3"
        )
        build_transcript_semantic_report(conn, run_id=result["run_id"], units=units)
    upsert_dashboard_payload(
        db_path=sb_db,
    ...
    base_report = build_mission_lens_report(
        itir_db_path=itir_db,
        sb_db_path=sb_db,
        date="2026-03-08",
    ...
    with sqlite3.connect(itir_db) as conn:
        ...
        current = load_mission_actual_mapping_current(
            conn, run_id="mission-lens-fixture-v3"
        )
...
def test_mission_lens_reassigned_mapping_becomes_effective_and_lexical_rows_explain_match(
    tmp_path,
) -> None:
    from scripts.mission_lens import build_mission_lens_report
    from sb.dashboard_store_sqlite import DashboardKey, upsert_dashboard_payload
    ...
    with sqlite3.connect(itir_db) as conn:
        ...
        units = [
            ...
            TextUnit(
                ...
                "chat-a",
                "chat_test_db",
                "[5/3/26 9:02 pm] Josh: Have you implemented the new feature?",
            ),
        ]
        result = run_transcript_semantic_pipeline(
            conn, units, run_id="mission-lens-fixture-v4"
        )
        build_transcript_semantic_report(conn, run_id=result["run_id"], units=units)
    upsert_dashboard_payload(
        db_path=sb_db,
    ...
    lexical_report = build_mission_lens_report(
        itir_db_path=itir_db,
        sb_db_path=sb_db,
        date="2026-03-08",
    ...
    assert lexical_row["recommendedAction"] in {
        "auto_link_safe",
...
def test_mission_lens_apply_safe_recommendations_only_applies_safe_rows(
    tmp_path,
) -> None:
    from scripts.mission_lens import (
        apply_safe_recommendations,
        build_mission_lens_report,
    )
    from sb.dashboard_store_sqlite import DashboardKey, upsert_dashboard_payload
    ...
    with sqlite3.connect(itir_db) as conn:
        ...
        units = [
            ...
            TextUnit(
                ...
                "chat-b",
                "chat_test_db",
                "[5/3/26 9:00 pm] Josh: We should track the new feature.",
            ),
        ]
        result = run_transcript_semantic_pipeline(
            conn, units, run_id="mission-lens-fixture-v5"
        )
        build_transcript_semantic_report(conn, run_id=result["run_id"], units=units)
    upsert_dashboard_payload(
        db_path=sb_db,
    ...
    before = build_mission_lens_report(
        itir_db_path=itir_db,
        sb_db_path=sb_db,
        date="2026-03-08",
    ...
    safe_before = [
        row
        for row in before["activity_rows"]
        if row["recommendedAction"] == "auto_link_safe"
        and row["recommendationConfidence"] == "high"
    ]
    assert safe_before
    ...
    after = build_mission_lens_report(
        itir_db_path=itir_db,
        sb_db_path=sb_db,
        date="2026-03-08",
        run_id="mission-lens-fixture-v5",
    ...
    effective_receipt_pairs = {
        (receipt["kind"], receipt["value"]) for receipt in effective_rows[0]["receipts"]
    }
    ...
    receipt_pairs = {
        ...
        for receipt in applied_history[0]["receipts"]
    }
...
def test_transcript_relation_summary_reports_counts_and_cues() -> None:
    ...
    result = run_transcript_semantic_pipeline(
        conn, units, run_id="transcript-fixture-v7"
    )
    from src.transcript_semantic.semantic import build_transcript_relation_summary

    summary = build_transcript_relation_summary(
        conn, run_id=result["run_id"], units=units
    )
    ...
    assert (
        summary["social_candidate_only_note"]
        == "All explicit social/care predicates remain candidate-only in this run."
    )
...
def test_transcript_semantic_report_builds_mission_observer_followup_refs() -> None:
    ...
    result = run_transcript_semantic_pipeline(
        conn, units, run_id="transcript-mission-fixture-v1"
    )
    report = build_transcript_semantic_report(
        conn, run_id=result["run_id"], units=units
    )
    ...
    resolved_followup = next(
        row for row in mission["followups"] if row["status"] == "linked"
    )
    assert resolved_followup["resolvedTopicLabel"] == "notification routing feature"
    assert resolved_followup["deadline"] == "Friday"
    overlay = next(
        row for row in mission["sb_observer_overlays"] if row["status"] == "linked"
    )
...
def test_transcript_report_emits_grouped_source_documents_and_source_spans() -> None:
    ...
    units = [
        TextUnit(
            "u1", "/tmp/demo-source.txt", "text_file", "Alice is the guardian of Carol."
        ),
    ...
    result = run_transcript_semantic_pipeline(
        conn, units, run_id="transcript-fixture-v8"
    )
    report = build_transcript_semantic_report(
        conn, run_id=result["run_id"], units=units
    )
    ...
    source_documents = {
        row["sourceDocumentId"]: row for row in report["source_documents"]
    }
    assert "/tmp/demo-source.txt" in source_documents
    assert "chat-run-1" in source_documents
    assert (
        "Alice is the guardian of Carol."
        in source_documents["/tmp/demo-source.txt"]["text"]
    )
    assert "Mary cared for Bob." in source_documents["/tmp/demo-source.txt"]["text"]

    debug_event = next(
        event for event in report["text_debug"]["events"] if event["eventId"] == "u1"
    )
