...
def test_numbering_changes_do_not_diff():
    ...
    ids_a = compute_identities(
        extract_obligations_from_text(a, references=[], source_id="doc")
    )
    ids_b = compute_identities(
        extract_obligations_from_text(b, references=[], source_id="doc")
    )
...
def test_actor_action_flags_toggle_identity_components():
    ...
    ids_with = compute_identities(
        extract_obligations_from_text(text, references=[], source_id="doc")
    )
