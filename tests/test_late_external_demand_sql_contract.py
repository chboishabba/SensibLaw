...
M097 = (
    ROOT
    / "database/postgres_migrations/097_external_evidence_projection_and_wakeup.sql"
)
M098 = (
    ROOT / "database/postgres_migrations/098_external_demand_hardening_and_receipts.sql"
)
M099 = ROOT / "database/postgres_migrations/099_external_fact_axis_reuse.sql"
M100 = (
    ROOT / "database/postgres_migrations/100_external_provider_boundary_projection.sql"
)
M101 = ROOT / "database/postgres_migrations/101_literal_provider_call_receipts.sql"
M103 = (
    ROOT
    / "database/postgres_migrations/103_external_source_freshness_and_snapshot_provenance.sql"
)
...
M108 = (
    ROOT / "database/postgres_migrations/108_freshness_filtered_external_projection.sql"
)
...
def test_expired_provider_lease_is_cache_probed_before_retry() -> None:
    ...
    assert (
        "request.request_state=4 AND request.lease_expires_at<CURRENT_TIMESTAMP"
        in refresh
    )
...
def test_provider_claim_projects_only_external_boundary_identifiers() -> None:
    ...
    assert (
        ...
        not in sql.split("RETURNS TABLE", 1)[1].split(") LANGUAGE", 1)[0]
    )
    assert (
...
def test_candidate_discovery_is_monotone_and_unknown_age_cannot_replace_known_age() -> (
    None
):
    sql = _sql(M105)
    ...
    assert (
        "EXCLUDED.source_epoch>=execution.semantic_pnf_label_world_candidate.source_epoch"
        in upsert
    )
    assert "OR EXCLUDED.source_epoch IS NULL" not in upsert


def test_freshness_recomputes_exact_active_member_max_and_handles_property_to_discovery_fan_in() -> (
    None
):
    sql = _sql(M106)
    recompute = sql.split(
...
def test_active_external_context_projection_uses_newest_admissible_epoch_only() -> None:
    ...
    assert (
        "DELETE FROM execution.semantic_pnf_world_candidate_requirement" in materialize
    )
    assert (
        "provider_property_numeric_id=request.provider_property_numeric_id"
        in materialize
    )
    assert "Manual/static candidate requirements and cold" in sql


def test_external_planner_fix_avoids_record_shadowing_and_short_circuits_empty_need_sets() -> (
    None
):
    sql = _sql(M110)
    body = sql.split(
