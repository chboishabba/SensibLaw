...
def test_name_expansion_cannot_bootstrap_identity_from_uniqueness_alone() -> None:
    policy = _text(M079)
    strong_base = policy.split("-- Strong parser evidence", 1)[1].split(
        "-- The target object", 1
    )[0]
    assert "candidate.witness_kind IN (2, 4, 6)" in strong_base
    assert "candidate.witness_kind IN (2, 3, 4, 6)" not in strong_base
...
def test_no_json_or_similarity_authority_is_introduced() -> None:
    folded = "\n".join(_text(path).casefold() for path in (M077, M078, M079))
