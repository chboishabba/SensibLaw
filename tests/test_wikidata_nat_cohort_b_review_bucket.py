...
def test_build_nat_cohort_b_review_bucket_emits_review_rows_and_variance_flags() -> (
    None
):
    ...
    assert (
        result["schema_version"] == WIKIDATA_NAT_COHORT_B_REVIEW_BUCKET_SCHEMA_VERSION
    )
    ...
    assert (
        ...
        in result["review_bucket_rows"][0]["variance_flags"]
    )
    assert (
        "unexpected_qualifier_properties"
        in result["review_bucket_rows"][1]["variance_flags"]
    )
    assert (
...
def test_build_nat_cohort_b_review_bucket_holds_when_payload_contains_out_of_lane_rows() -> (
    None
):
    payload = {
