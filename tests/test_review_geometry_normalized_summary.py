...
def test_build_review_geometry_normalized_summary(tmp_path: Path) -> None:
    ...
    assert (
        metrics_by_id["au_narrow_affidavit_coverage_review_v1"][
            "dominant_primary_workload"
        ]
        == "queue_pressure"
    )
    assert (
        metrics_by_id["au_dense_affidavit_coverage_review_v1"][
            "review_required_source_ratio"
        ]
        == 0.875
    )
    assert (
        metrics_by_id["wikidata_checked_structural_review_v1"][
            "primary_workload_counts"
        ]["governance_pressure"]
        == 3
    )
    assert (
        metrics_by_id["wikidata_dense_structural_review_v1"]["candidate_signal_density"]
        == 1.4
    )
    assert (
        metrics_by_id["gwb_checked_public_review_v1"]["source_status_counts"][
            "review_required"
        ]
        == 45
    )
