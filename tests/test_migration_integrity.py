...
def test_all_migrations_applied_in_memory():
    ...
    assert applied_files == expected_files, (
        f"Missing or extra migrations: {expected_files - applied_files} / {applied_files - expected_files}"
    )
...
def test_read_model_migration_list_sync():
    ...
    assert not missing_in_code, (
        f"Migrations found in directory but missing from read_model.py _FACT_INTAKE_MIGRATION_FILES: {missing_in_code}"
    )
    ...
    assert not missing_on_disk, (
        f"Migrations listed in read_model.py but missing from disk: {missing_on_disk}"
    )
