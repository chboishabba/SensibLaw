...
from .latent_promoted_graph import (
...
def _build_record_ref(system_id: str, row: Mapping[str, Any]) -> str:
    ...
    subject_key = (
        _required_str(row["subject"], "canonical_key")
        if isinstance(row.get("subject"), Mapping)
        else ""
    )
    object_key = (
        _required_str(row["object"], "canonical_key")
        if isinstance(row.get("object"), Mapping)
        else ""
    )
    digest = _stable_hash(
        (system_id, event_id, predicate_key, subject_key, object_key)
    )[:16]
    return f"promoted://{system_id}/relation/{digest}"


def _extract_promoted_records(
    system_id: str, report: Mapping[str, Any]
) -> list[dict[str, Any]]:
    ...
    for row in promoted_relations:
        ...
        if event_row is None:
            raise ValueError(
                f"promoted relation {event_id} is missing a per_event anchor row"
            )
        ...
        records.append(
            {
                ...
                "subject_kind": _top_level_kind(
                    _required_str(subject, "canonical_key")
                ),
    ...
    if not records:
        raise ValueError(
            f"report for {system_id} does not contain promoted_true relations"
        )
    return records


def extract_promoted_records_from_report(
    *, system_id: str, report: Mapping[str, Any]
) -> list[dict[str, Any]]:
...
def _mapping_score(source: Mapping[str, Any], target: Mapping[str, Any]) -> float:
    same_shape = (
        source["subject_kind"] == target["subject_kind"]
        and source["object_kind"] == target["object_kind"]
    )
    same_rule = (
        source.get("rule_type") == target.get("rule_type")
        and source.get("rule_type") is not None
    )
...
def _build_mapping_explanation(
    *,
    ...
    witness = (
        meta_validation.get("witness", {})
        if isinstance(meta_validation.get("witness"), Mapping)
        else {}
    )
    type_alignment = (
        witness.get("type_alignment", {})
        if isinstance(witness.get("type_alignment"), Mapping)
        else {}
    )
    authority_alignment = (
    ...
    constraint_check = (
        witness.get("constraint_check", {})
        if isinstance(witness.get("constraint_check"), Mapping)
        else {}
    )
    scope_check = (
        witness.get("scope_check", {})
        if isinstance(witness.get("scope_check"), Mapping)
        else {}
    )
    return {
        ...
        "meta_summary": {
            ...
            "constraint_status": constraint_check.get(
                "status", meta_validation.get("constraint_status")
            ),
            "scope_status": scope_check.get(
                "status", meta_validation.get("scope_status")
            ),
        },
        "witness": witness,
        "latent_graph_refs": {
            "source_fact_node_ref": source_graph_refs.get("fact_node_ref")
            if isinstance(source_graph_refs, Mapping)
            else None,
            "target_fact_node_ref": target_graph_refs.get("fact_node_ref")
            if isinstance(target_graph_refs, Mapping)
            else None,
            "source_motif_ref": (
                source_graph_refs.get("motif_node_refs", [None])[0]
                if isinstance(source_graph_refs, Mapping)
                and isinstance(source_graph_refs.get("motif_node_refs"), list)
                and source_graph_refs.get("motif_node_refs")
                else None
            ),
            "target_motif_ref": (
                target_graph_refs.get("motif_node_refs", [None])[0]
                if isinstance(target_graph_refs, Mapping)
                and isinstance(target_graph_refs.get("motif_node_refs"), list)
                and target_graph_refs.get("motif_node_refs")
                else None
            ),
        },
    }
...
def _find_best_target(
    source: Mapping[str, Any], targets: list[Mapping[str, Any]]
...
def build_cross_system_phi_prototype(
    *,
    ...
    meta_contract = build_default_phi_meta_contract(
        left_system=source_system_id, right_system=target_system_id
    )
    ...
    source_basis_ref = (
        f"promoted://{source_system_id}/run/{_required_str(source_report, 'run_id')}"
    )
    target_basis_ref = (
        f"promoted://{target_system_id}/run/{_required_str(target_report, 'run_id')}"
    )
    ...
    source_graph_index = {
        row["record_ref"]: row for row in source_graph["record_index"]
    }
    target_graph_index = {
        row["record_ref"]: row for row in target_graph["record_index"]
    }
    ...
    for index, source_record in enumerate(source_records, start=1):
        available_targets = [
            row for row in target_records if row["record_ref"] not in used_target_refs
        ]
        ...
        for candidate_target in available_targets:
            ...
            if meta_validation["allowed"]:
                allowed_targets.append(
                    {**candidate_target, "meta_validation": meta_validation}
                )
                continue
            blocked_pairs.append(
                {
                    ...
                    "reason_summary": "; ".join(meta_validation["violations"])
                    or "meta validation failed",
            ...
            if status in {"partial", "incompatible"}:
                ...
                if status == "incompatible":
                    ...
                    constraint_refs.append(
                        "constraint://target/no_transfer_without_manual_reclassification"
                    )
                    summary = (
                    ...
                    constraint_refs.append(
                        "constraint://target/manual_scope_review_required"
                    )
                    summary = (
        ...
        mappings.append(
            {
                ...
                "mapping_explanation": _build_mapping_explanation(
                    ...
                    rationale=rationale,
                    source_graph_refs=source_graph_index.get(
                        source_record["record_ref"]
                    ),
                    target_graph_refs=target_graph_index.get(
                        target_record["record_ref"]
                    )
