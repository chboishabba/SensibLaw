...
def import_au_semantic_seed_payload(
    ...
    if seed_ids:
        ...
        conn.execute(
            f"DELETE FROM au_semantic_linkage_match_receipts WHERE seed_id IN ({placeholders})",
            seed_ids,
        )
        conn.execute(
            f"DELETE FROM au_semantic_linkage_matches WHERE seed_id IN ({placeholders})",
            seed_ids,
        )
        conn.execute(
            f"DELETE FROM au_semantic_linkage_seed_authorities WHERE seed_id IN ({placeholders})",
            seed_ids,
        )
        conn.execute(
            f"DELETE FROM au_semantic_linkage_seed_refs WHERE seed_id IN ({placeholders})",
            seed_ids,
        )
        conn.execute(
            f"DELETE FROM au_semantic_linkage_seed_cues WHERE seed_id IN ({placeholders})",
            seed_ids,
        )
        conn.execute(
            f"DELETE FROM au_semantic_linkage_seeds WHERE seed_id IN ({placeholders})",
            seed_ids,
        )
    imported = 0
    for item in items:
        ...
        for ref_kind, field_name in (
            ("institution_ref", "institution_refs"),
            ("court_ref", "court_refs"),
        ):
            ref_order = 0
            for ref in (
                ...
                if canonical_ref:
                    for normalized_kind, normalized_ref in _normalized_seed_refs(
                        ref_kind, canonical_ref
                    ):
                        ref_order += 1
                        conn.execute(
...
def _normalized_seed_refs(
    ref_kind: str, canonical_ref: str
) -> tuple[tuple[str, str], ...]:
    return _AU_REF_NORMALIZATION.get(
        (ref_kind, canonical_ref), ((ref_kind, canonical_ref),)
    )
...
def run_au_semantic_linkage(
    ...
    *,
    ...
    conn.execute(
        "DELETE FROM au_semantic_linkage_match_receipts WHERE run_id = ?",
        (active_run_id,),
    )
    conn.execute(
        "DELETE FROM au_semantic_linkage_matches WHERE run_id = ?", (active_run_id,)
    )
    seeds = conn.execute(
        """
    ...
    for row in authority_rows:
        authorities.setdefault(str(row["seed_id"]), []).append(
            str(row["authority_title"])
        )
    for row in ref_rows:
        refs.setdefault(str(row["seed_id"]), []).append(
            (str(row["ref_kind"]), str(row["canonical_ref"]))
        )
    for row in cue_rows:
    ...
    for event in events:
        ...
        top_count = sum(
            1 for _, score, _, _ in scored if score == top_score and top_score > 0
        )
        if top_count > 1:
            ...
            conn.execute(
                ...
                (
                    ...
                    if matched
                        ...
                        "abstain"
                        if top_count > 1 or confidence == "low"
                        else confidence
                    ),
                    matched,
                    score,
...
def build_au_semantic_linkage_report(
        ...
        receipts_map.setdefault(key, []).append(
            {
                ...
                "reason_value": str(row["reason_value"]),
            }
        )
    ...
    matched_seed_ids = {
        entry["seed_id"]
        for event in per_event.values()
        for entry in event["matches"]
        if entry["matched"]
    }
    unmatched = [
        row["seed_id"] for row in per_seed if row["seed_id"] not in matched_seed_ids
    ]
    ambiguous = [
        event
        for event in per_event.values()
        if any(match["ambiguity_group"] for match in event["matches"])
    ]
    return {
        "run_id": run_id,
        "per_seed": per_seed,
