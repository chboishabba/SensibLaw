...
def _build_claims(
    ...
    claims = build_review_claim_records(
        ...
        mapping=ReviewClaimRecordMapping(
            ...
            provenance_chain=lambda row, context: {
                ...
                "event_ids": [
                    _text(value) for value in row.get("event_ids", []) if _text(value)
                ],
            },
            canonical_form=lambda row, context: {
                ...
                "qualifiers": {
                    ...
                    if _text(row.get("candidate_status"))
                    else [],
                    "reason_codes": [
                        _text(value)
    ...
    summary = {
        ...
        "must_review_count": sum(
            1
            for claim in claims
            if _text((claim.get("action_policy") or {}).get("actionability"))
            == "must_review"
        ),
    }
            ...
            "linkage_inputs": build_review_inputs(
                ...
                extra_fields={
                    ...
                    "operator_workflow_surface": deepcopy(
                        dict(operator_workflow_surface)
                    ),
...
def project_report(world_model: Mapping[str, Any]) -> dict[str, Any]:
    ...
    review_surface = project_review_surface(
        ...
        summary={
            "review_row_count": int(
                (model.get("summary") or {}).get("claim_count") or 0
            )
        },
        metadata={
            "linkage_inputs": deepcopy(dict(metadata.get("linkage_inputs") or {}))
        },
    )
    claim_table = project_claim_table(
        ...
        summary={
            "row_count": int((model.get("summary") or {}).get("claim_count") or 0)
        },
    )
    linkage_case_payload = build_linkage_case(
        {
            "world_model_ref": model,
            **deepcopy(dict(metadata.get("linkage_inputs") or {})),
        }
    )
    ...
    return _project_report(
        ...
        extra_fields={
            ...
            "convergence_schema_version": _text(
            ...
            "action_policy_schema_version": _text(
