...
def build_operator_workflow_surface(
    *,
    ...
    counts = (
        workflow.get("counts") if isinstance(workflow.get("counts"), Mapping) else {}
    )
    ...
    return {
        ...
        "lane": str(
            normalized_contract.get("lane") or normalized_gate.get("lane") or ""
        ).strip(),
        ...
        "summary": {
            ...
            "evidence_bundle_kind": str(
                evidence_bundle.get("bundle_kind") or ""
            ).strip(),
            "evidence_source_family": str(
                evidence_bundle.get("source_family") or ""
            ).strip(),
