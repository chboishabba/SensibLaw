...
RowContextExtractor = (
    str | Callable[[Mapping[str, Any], Mapping[str, Any]], AdapterValue]
)
...
def copy_artifact_fields(
    artifact: Mapping[str, Any], *field_names: str
) -> dict[str, Any]:
    copied: dict[str, Any] = {}
    for field_name in field_names:
        ...
        if isinstance(value, Mapping):
            ...
            copied[field_name] = [
                deepcopy(dict(row)) if isinstance(row, Mapping) else row
                for row in value
            ]
        else:
...
def build_authority_surface_rows(surface_ids: Sequence[str]) -> list[dict[str, Any]]:
    return [
        {"surface_id": _text(value), "status": "reviewed"}
...
def build_event_nodes_from_mapping(
    ...
    *,
    ...
    return build_record_rows_from_mapping(
        rows, field_mapping=field_mapping, context=context, predicate=predicate
    )
    ...
    for row in rows:
        ...
        built.append(
            build_state_node(
                ...
                status=_text(_extract(mapping.status, row, normalized_context))
                or "candidate",
                source_anchor_ids=_string_list(
                    _extract(mapping.source_anchor_ids, row, normalized_context)
                    if mapping.source_anchor_ids
                    else []
                ),
                authority_surface=_text(
                    _extract(mapping.authority_surface, row, normalized_context)
                    if mapping.authority_surface
                    else ""
                )
                or None,
                promotion_status=_text(
                    _extract(mapping.promotion_status, row, normalized_context)
                    if mapping.promotion_status
                    else ""
                )
                or None,
                conflict_ids=_string_list(
                    _extract(mapping.conflict_ids, row, normalized_context)
                    if mapping.conflict_ids
                    else []
                ),
                residual=_deepcopy_mapping(
                    _extract(mapping.residual, row, normalized_context)
                    if mapping.residual
                    else {}
                )
                or None,
                metadata=_deepcopy_mapping(
                    _extract(mapping.metadata, row, normalized_context)
                    if mapping.metadata
                    else {}
                ),
            )
    ...
    *,
    ...
    for row in rows:
        ...
        candidate_id = (
            _text(
                _extract(
                    mapping.candidate_id or mapping.claim_id, row, normalized_context
                )
            )
            or claim_id
        )
        cohort_id = _text(
            _extract(
                mapping.cohort_id or mapping.root_artifact_id, row, normalized_context
            )
        )
        root_artifact_id = _text(
            _extract(mapping.root_artifact_id, row, normalized_context)
        )
        ...
        authority_level = _text(
            _extract(mapping.authority_level, row, normalized_context)
        )
        claim_status = (
            _text(_extract(mapping.claim_status, row, normalized_context))
            or "REVIEW_ONLY"
        )
        evidence_status = (
            _text(_extract(mapping.evidence_status, row, normalized_context))
            or "review_only"
        )
        provenance_chain = deepcopy(
            dict(mapping.provenance_chain(row, normalized_context))
        )
        evidence_paths = [
            {
        ...
        claim["nat_claim"] = build_nat_claim_dict(
            ...
            candidate_id=candidate_id,
            canonical_form=canonical_form,
            source_property=_text(
                _extract(mapping.source_property, row, normalized_context)
            ),
            target_property=_text(
                _extract(mapping.target_property, row, normalized_context)
            ),
            state="review_claim",
    ...
    *,
    ...
    for row in rows:
        ...
        source_unit_id = (
            _text(
                _extract(
                    mapping.source_unit_id or mapping.claim_id, row, normalized_context
                )
            )
            or claim_id
        )
        run_id = (
            _text(
                _extract(
                    mapping.run_id or mapping.root_artifact_id, row, normalized_context
                )
            )
            or root_artifact_id
        )
        ...
        nat_claim_state = (
            _text(_extract(mapping.nat_claim_state, row, normalized_context))
            or claim_status
        )
        provenance_chain = deepcopy(
            dict(mapping.provenance_chain(row, normalized_context))
        ...
        claim = {
            ...
            "evidence_count": len(evidence_paths),
            "state_basis": _text(
                _extract(mapping.state_basis, row, normalized_context)
            ),
        }
        claim["nat_claim"] = build_nat_claim_dict(
            claim_id=claim_id,
