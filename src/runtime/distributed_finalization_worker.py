...
DISTRIBUTED_FINALIZATION_WORKER_CONTRACT = "postgres-fenced-finalization-worker:v1"
...
class FinalizationLeaseHeartbeat:
                ...
                with self.connection_factory() as connection:
                    with connection.cursor() as cursor:
                        ...
                        if cursor.rowcount != 1:
                            raise StaleSemanticLeaseError(self.lease.checkpoint_ref)
    ...
    def assert_healthy(self) -> None:
        if self.error is not None:
            raise StaleSemanticLeaseError(self.lease.checkpoint_ref) from self.error
...
class DistributedFinalizationWorker:
        ...
        with self.connection_factory() as connection:
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    WHERE document_ref = %s AND state_ref = 'leased'
    ...
    def _complete(
        self, lease: FinalizationLease, result: FinalizationBatchResult
    ) -> None:
        with self.connection_factory() as connection:
            with connection.cursor() as cursor:
                self.store.complete_finalization_checkpoint(
                    ...
                    metrics={
                        "worker_contract_ref": DISTRIBUTED_FINALIZATION_WORKER_CONTRACT,
                        **dict(result.metrics),
                    },
                )
