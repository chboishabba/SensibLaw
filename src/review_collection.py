...
def load_collection(path: Path) -> dict:
    ...
    if data.get("version") != COLLECTION_VERSION:
        raise ValueError(
            f"Expected version {COLLECTION_VERSION}, got {data.get('version')}"
        )
    return data
...
def manifest(collection_path: Path) -> dict:
    ...
    for item in col["bundles"]:
        ...
        bpath = (
            (collection_path.parent / item_path).resolve()
...
def validate_collection_schema(
    collection: Mapping[str, Any], schema_path: Path
) -> None:
...
def export_collection(collection_path: Path, out_zip: Path) -> dict:
    ...
    collection_for_export = {
        ...
        "bundles": [
            {k: v for k, v in bundle.items() if k != "hash"}
    ...
    with zipfile.ZipFile(out_zip, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        zf.writestr(
            ...
            json.dumps(
                collection_for_export, ensure_ascii=False, indent=2, sort_keys=True
            ),
        )
        zf.writestr(
        ...
        for item in canonical_bundles:
            ...
            bpath = (
                (collection_path.parent / rel_path).resolve()
