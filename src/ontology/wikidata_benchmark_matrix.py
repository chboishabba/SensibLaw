...
def run_benchmark_matrix(
    ...
    *,
    ...
    lane_rows = [
        _evaluate_lane(lane, manifest, network_mode=network_mode) for lane in selected
    ]
...
def _evaluate_lane(
    lane: str, manifest: dict[str, Any], *, network_mode: str
) -> dict[str, Any]:
    ...
    for key in (
        ...
        if key in expected and observed.get(key) != expected.get(key):
            severity = "regression"
            if key == "provenance_count" and int(observed.get(key) or 0) >= int(
                expected.get(key) or 0
            ):
                severity = "ok"
