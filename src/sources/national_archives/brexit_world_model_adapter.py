...
def build_world_model(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if _as_text(payload.get("fixture_kind")) != "gwb_broader_review":
        raise ValueError(
            "Brexit world-model adapter requires gwb_broader_review fixture kind"
        )
    ...
    claims = build_review_claim_records(
        ...
        mapping=ReviewClaimRecordMapping(
            claim_id=lambda row, _context: (
                f"brexit-review:{_as_text(row.get('source_row_id'))}"
            ),
            candidate_id=lambda row, _context: (
                f"brexit-review:{_as_text(row.get('source_row_id'))}"
            ),
            cohort_id=lambda _row, context: context.get("artifact_id"),
            root_artifact_id=lambda _row, context: context.get("artifact_id"),
            source_family=lambda row, _context: (
                _as_text(row.get("source_family")) or "brexit_review_row"
            ),
            authority_level=lambda row, _context: (
                _as_text(row.get("source_kind")) or "brexit_review_row"
            ),
            claim_status=lambda row, _context: _claim_status(
                _as_text(row.get("review_status"))
            ),
            evidence_status=lambda row, _context: (
                _as_text(row.get("review_status")) or "review_required"
            ),
    ...
    claims.extend(
        build_review_claim_records(
            ...
            mapping=ReviewClaimRecordMapping(
                claim_id=lambda row, _context: (
                    f"brexit-archive:{_as_text(row.get('doc_id'))}"
                ),
                candidate_id=lambda row, _context: (
                    f"brexit-archive:{_as_text(row.get('doc_id'))}"
                ),
                ...
                evidence_status=lambda row, _context: (
                    "archive_follow_live"
                    if row.get("live_fetch")
                    else "archive_follow_fixture"
                ),
                source_property=lambda _row, _context: "brexit_review",
