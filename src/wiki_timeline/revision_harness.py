...
from src.text.similarity import (
    simhash,
...
def _article_identity(
    ...
    prev_source = (
        previous_payload.get("source_entity")
        if isinstance(previous_payload, dict)
        else None
    )
    curr_source = (
        current_payload.get("source_entity")
        if isinstance(current_payload, dict)
        else None
    )
    wiki = _safe_str(current_snapshot.get("wiki")) or _safe_str(
        previous_snapshot.get("wiki")
    )
    title = _safe_str(current_snapshot.get("title")) or _safe_str(
        previous_snapshot.get("title")
    )
    source_url = (
...
def _extract_source_text(
    snapshot: Mapping[str, Any], payload: Mapping[str, Any] | None
) -> str:
    ...
    if isinstance(payload, Mapping):
        ...
        if isinstance(events, list):
            event_texts = [
                _norm_text(ev.get("text")) for ev in events if isinstance(ev, Mapping)
            ]
...
def _observation_map(state: Mapping[str, Any] | None) -> dict[str, dict[str, Any]]:
    ...
    for index, observation in enumerate(observations):
        if not isinstance(observation, dict):
            continue
        observation_id = (
            _safe_str(observation.get("observation_id")) or f"observation:{index}"
        )
...
def _normalize_step_signature(step: Any) -> dict[str, Any] | None:
    ...
    return {
        ...
        "subjects": _sorted_unique(
            [_safe_str(v) or "" for v in (step.get("subjects") or []) if _safe_str(v)]
        ),
        "objects": _sorted_unique(
            [
                _normalize_object(v) or ""
                for v in (step.get("objects") or [])
                if _normalize_object(v)
            ]
        ),
...
def _event_signature(event: Mapping[str, Any]) -> dict[str, Any]:
    ...
    attrs = [
        _normalize_attribution_signature(item)
        for item in (event.get("attributions") or [])
    ]
    return {
        "text": _norm_text(event.get("text")),
        "action": _safe_str(event.get("action")),
        "claim_bearing": bool(event.get("claim_bearing")),
        "claim_step_indices": sorted(
            int(v)
            for v in (event.get("claim_step_indices") or [])
            if isinstance(v, int)
        ),
        "actors": _sorted_unique(
            [
                _normalize_actor(actor) or ""
                for actor in (event.get("actors") or [])
                if _normalize_actor(actor)
            ]
        ),
        "objects": _sorted_unique(
            [
...
def _text_unit_signature(unit: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "anchor": dict(unit.get("anchor"))
        if isinstance(unit.get("anchor"), Mapping)
        else None,
    }
...
def _changed_ids_by_signature(
    ...
    for item_id in sorted(prev_ids & curr_ids):
        if _stable_json(signature_fn(previous_map[item_id])) != _stable_json(
            signature_fn(current_map[item_id])
        ):
...
def _event_related_entities(
    previous_event: Mapping[str, Any] | None, current_event: Mapping[str, Any] | None
) -> list[str]:
    values: list[str] = []
...
def _event_surface_diff(
    ...
    if previous_event is None or current_event is None:
        ...
        if (previous_event or current_event) and bool(
            (previous_event or current_event).get("claim_bearing")
        ):
    ...
    if (
        prev_sig["actors"] != curr_sig["actors"]
        or prev_sig["steps"] != curr_sig["steps"]
    ):
        surfaces.append("semantic")
        reasons.append("actor/step extraction changed")
    if (
        prev_sig["objects"] != curr_sig["objects"]
        or prev_sig["action"] != curr_sig["action"]
    ):
    ...
    if (
        prev_sig["anchor_status"] != curr_sig["anchor_status"]
        or prev_sig["anchor"] != curr_sig["anchor"]
    ):
        surfaces.append("logical")
...
def _event_state_change_summary(
    previous_event: Mapping[str, Any] | None, current_event: Mapping[str, Any] | None
) -> list[str]:
    changes: list[str] = []
...
def build_revision_comparison_report(
    *,
    ...
    article = _article_identity(
        previous_snapshot_obj, current_snapshot_obj, previous_payload, current_payload
    )

    previous_state = coerce_wiki_article_state(
        snapshot=previous_snapshot_obj, payload=previous_payload
    )
    current_state = coerce_wiki_article_state(
        snapshot=current_snapshot_obj, payload=current_payload
    )
    ...
    added_observation_ids, removed_observation_ids, changed_observation_ids = (
        _changed_ids_by_signature(
            previous_observations,
            current_observations,
            _observation_signature,
        )
    )
    added_event_ids, removed_event_ids, changed_event_ids = _changed_event_ids(
    ...
    prev_timeline_rows = (
        [
            row
            for row in previous_state.get("timeline_projection") or []
    ...
    curr_timeline_rows = (
        [
            row
            for row in current_state.get("timeline_projection") or []
            if isinstance(row, Mapping)
        ]
    ...
    prev_anchor_status = {
        str(row.get("event_id") or ""): str(row.get("anchor_status") or "none")
        for row in prev_timeline_rows
    }
    curr_anchor_status = {
        str(row.get("event_id") or ""): str(row.get("anchor_status") or "none")
        for row in curr_timeline_rows
    }
    ...
    for event_id in packet_ids:
        ...
        packet = {
            ...
            "attribution_changed": bool(
                prev_sig["attributions"] != curr_sig["attributions"]
            ),
    ...
    return {
        ...
        "revisions": {
            "same_article": (
                ...
                and (
                    _safe_str(previous_snapshot_obj.get("title"))
                    in {None, article.get("title")}
                    and _safe_str(current_snapshot_obj.get("title"))
                    in {None, article.get("title")}
                )
            ),
        ...
        "similarities": {
            "source_text": _similarity_summary(
                source_text_previous, source_text_current
            ),
            "extraction_text": _similarity_summary(
                extraction_text_previous, extraction_text_current
            ),
        },
        "state_delta_summary": {
        ...
        "epistemic_delta_summary": {
            ...
            "added_claim_bearing_event_ids": sorted(
                set(curr_claim_ids) - set(prev_claim_ids)
            ),
            "removed_claim_bearing_event_ids": sorted(
                set(prev_claim_ids) - set(curr_claim_ids)
            ),
            ...
            "changed_attribution_event_ids": sorted(
                {
                    ...
                    for event_id in packet_ids
                    if "epistemic"
                    in _event_surface_diff(
                        previous_events.get(event_id), current_events.get(event_id)
                    )[0]
                }
            ),
        },
        ...
        "triage_dashboard": {
            ...
            "top_packet_ids": [
                str(packet["packet_id"]) for packet in issue_packets[:5]
            ],
            "highest_severity": (
                "high"
