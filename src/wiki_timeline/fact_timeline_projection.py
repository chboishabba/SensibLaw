...
def _normalize_anchor(raw: Any) -> dict[str, Any]:
    return {
        ...
        "month": int(raw["month"])
        if isinstance(raw, dict) and raw.get("month") is not None
        else None,
        "day": int(raw["day"])
        if isinstance(raw, dict) and raw.get("day") is not None
        else None,
        "precision": str(raw.get("precision") or "year")
...
def _same_anchor(a: dict[str, Any], b: dict[str, Any]) -> bool:
    return (
        ...
        and (a.get("month") if a.get("month") is not None else -1)
        == (b.get("month") if b.get("month") is not None else -1)
        and (a.get("day") if a.get("day") is not None else -1)
        == (b.get("day") if b.get("day") is not None else -1)
    )


def _key_for_anchor(anchor: dict[str, Any]) -> int:
    return (
        (int(anchor.get("year") or 9999) * 10_000)
        + ((anchor.get("month") if anchor.get("month") is not None else 99) * 100)
...
def _fact_identity_key(row: dict[str, Any]) -> str:
    ...
    payload = {
        ...
        "negation_kind": _norm_text(
            (row.get("negation") or {}).get("kind")
            if isinstance(row.get("negation"), dict)
            else ""
        ),
        "subjects": _sorted_unique(
            [_norm_determiner(v) for v in row.get("subjects") or []]
        ),
        "objects": _sorted_unique(
            [_norm_determiner(v) for v in row.get("objects") or []]
        ),
        "chain_kinds": _sorted_unique(
            [_norm_text(v) for v in row.get("chain_kinds") or []]
        ),
    }
...
def _coalesce_fact_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if current is None:
            ...
            fact_id_alias[str(row.get("fact_id") or "")] = str(
                base.get("fact_id") or ""
            )
            continue
        fact_id_alias[str(row.get("fact_id") or "")] = str(current.get("fact_id") or "")
        current["prev_fact_ids"] = _sorted_unique(
            [*(current.get("prev_fact_ids") or []), *(row.get("prev_fact_ids") or [])]
        )
        current["next_fact_ids"] = _sorted_unique(
            [*(current.get("next_fact_ids") or []), *(row.get("next_fact_ids") or [])]
        )
        current["chain_kinds"] = _sorted_unique(
            [*(current.get("chain_kinds") or []), *(row.get("chain_kinds") or [])]
        )
    ...
    for row in out:
        ...
        row["prev_fact_ids"] = [
            ...
            for item in _sorted_unique(
                [
                    fact_id_alias.get(str(v), str(v))
                    for v in row.get("prev_fact_ids") or []
                ]
            )
        ...
        row["next_fact_ids"] = [
            ...
            for item in _sorted_unique(
                [
                    fact_id_alias.get(str(v), str(v))
                    for v in row.get("next_fact_ids") or []
                ]
            )
            if item != self_id and item in canon_ids
        ]
...
def _coerce_fact_row(
    ...
    use_event = (not mention_valid) and bool(
        event_anchor and int(event_anchor.get("year") or 0) > 0
    )
    primary_anchor = (
        mention_anchor if mention_valid else (event_anchor or mention_anchor)
    )
    event_out = (
        event_anchor
        if event_anchor and not _same_anchor(primary_anchor, event_anchor)
        else None
    )
    negation = (
        raw.get("negation")
        if isinstance(raw.get("negation"), dict)
        and isinstance(raw.get("negation", {}).get("kind"), str)
        else None
    )
    return {
        ...
        "subjects": [
            str(value) for value in raw.get("subjects") or [] if str(value).strip()
        ],
        ...
        "objects": [
            str(value) for value in raw.get("objects") or [] if str(value).strip()
        ],
        "purpose": str(raw.get("purpose"))
        if isinstance(raw.get("purpose"), str)
        else None,
        "text": str(raw.get("text") or text_by_event.get(event_id) or ""),
        "section": str(section_by_event.get(event_id) or ""),
        "prev_fact_ids": [
            str(value) for value in raw.get("prev_fact_ids") or [] if str(value).strip()
        ],
        "next_fact_ids": [
            str(value) for value in raw.get("next_fact_ids") or [] if str(value).strip()
        ],
        "chain_kinds": [
            str(value) for value in raw.get("chain_kinds") or [] if str(value).strip()
        ],
    }
...
def _synthesize_facts_from_events(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for event in payload.get("events") or []:
        ...
        steps = (
            event.get("steps")
            if isinstance(event.get("steps"), list) and event.get("steps")
            else [
                {
                    ...
                    "subjects": [
                        ...
                        and str(actor.get("role") or "") != "requester"
                    ],
                    "objects": [
                        str(obj.get("title"))
                        for obj in objects
        ...
        for idx, step in enumerate(steps):
            if not isinstance(step, dict):
                continue
            subjects = [
                str(value) for value in step.get("subjects") or [] if str(value).strip()
            ]
            objects_out = [
                str(value) for value in step.get("objects") or [] if str(value).strip()
            ]
            action = (
                str(step.get("action")) if isinstance(step.get("action"), str) else None
            )
            ...
            row = {
                ...
                if isinstance(step.get("negation"), dict)
                and isinstance(step.get("negation", {}).get("kind"), str)
                else None,
                "objects": objects_out,
                "purpose": str(step.get("purpose"))
                if isinstance(step.get("purpose"), str)
                else event.get("purpose"),
        ...
        for idx in range(len(event_rows) - 1):
            ...
            current["next_fact_ids"] = list(
                dict.fromkeys(
                    [*(current.get("next_fact_ids") or []), str(nxt["fact_id"])]
                )
            )
            current["chain_kinds"] = list(
                dict.fromkeys([*(current.get("chain_kinds") or []), "sequence"])
            )
            nxt["prev_fact_ids"] = list(
                dict.fromkeys(
                    [*(nxt.get("prev_fact_ids") or []), str(current["fact_id"])]
                )
            )
        if isinstance(event.get("chains"), list):
            for chain in event["chains"]:
                ...
                from_row["next_fact_ids"] = list(
                    dict.fromkeys([*(from_row.get("next_fact_ids") or []), to_fact])
                )
                from_row["chain_kinds"] = list(
                    dict.fromkeys([*(from_row.get("chain_kinds") or []), kind])
                )
                to_row["prev_fact_ids"] = list(
                    dict.fromkeys([*(to_row.get("prev_fact_ids") or []), from_fact])
                )
...
def _coerce_proposition(raw: Any) -> dict[str, Any] | None:
    ...
    return {
        ...
        "source_fact_id": str(raw.get("source_fact_id"))
        if isinstance(raw.get("source_fact_id"), str)
        else None,
        "source_signal": str(raw.get("source_signal"))
        if isinstance(raw.get("source_signal"), str)
        else None,
        "anchor_text": str(raw.get("anchor_text"))
        if isinstance(raw.get("anchor_text"), str)
        else None,
        "arguments": [
            {"role": str(arg.get("role")), "value": str(arg.get("value"))}
            for arg in raw.get("arguments") or []
            if isinstance(arg, dict)
            and isinstance(arg.get("role"), str)
        ...
        "receipts": [
            ...
            and isinstance(receipt.get("kind"), str)
            and isinstance(receipt.get("value"), str)
        ],
    }
...
def _coerce_proposition_link(raw: Any) -> dict[str, Any] | None:
    ...
    if (
        not link_id
        or not event_id
        or not source_proposition_id
        or not target_proposition_id
        or not link_kind
    ):
        return None
    return {
...
def build_fact_timeline_projection(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    section_by_event = {
        str(event.get("event_id") or ""): str(event.get("section") or "")
        for event in events
        if isinstance(event, dict)
    }
    text_by_event = {
        str(event.get("event_id") or ""): str(event.get("text") or "")
        for event in events
        if isinstance(event, dict)
    }
    ...
    facts = _coalesce_fact_rows(
        sorted(
            [
                _coerce_fact_row(
                    fact, section_by_event, text_by_event, event_anchor_by_event
                )
                for fact in raw_facts
                if isinstance(fact, dict)
                and str(fact.get("fact_id") or "")
                and str(fact.get("event_id") or "")
            ],
            key=lambda row: (
                _key_for_anchor(row["anchor"]),
    ...
    propositions = [
        row
        for row in (
            _coerce_proposition(item) for item in payload.get("propositions") or []
        )
        if row is not None
    ]
    proposition_links = [
        row
        for row in (
            _coerce_proposition_link(item)
            for item in payload.get("proposition_links") or []
        )
        if row is not None
    ]
