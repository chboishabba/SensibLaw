...
_PROJECTOR_CACHE: Callable[[str], list[dict[str, Any]]] | None | object = (
    _PROJECTOR_UNSET
)
...
def compile_turn(
    ...
    turn_id = str(
        payload.get("turn_id")
        or stable_id("turn", {"text": text, "meta": payload.get("metadata", {})})
    )

    source = _source_record(turn_id, text, payload)
    with _MetricSpan(
        metrics_callback, "segmentation", turn_id=turn_id, input_chars=len(text)
    ):
    ...
    atoms, pnfs, projection_abstentions = _predicate_records(
        statements, payload, metrics_callback, turn_id
    )
    with _MetricSpan(
        metrics_callback,
        "residual_pair_construction",
        turn_id=turn_id,
    ...
    with _MetricSpan(
        metrics_callback, "gate_construction", turn_id=turn_id, atom_count=len(atoms)
    ):
    ...
    if not atoms:
        abstentions.append(
            {
                "id": stable_id(
                    "abstain", {"turn_id": turn_id, "reason": "no-predicate-atoms"}
                ),
                "status": "abstained",
    ...
    delta_body = {
        ...
        "id": stable_id(
            "delta",
            {"turn_id": turn_id, "text": text, "metadata": payload.get("metadata", {})},
        ),
        "turn_id": turn_id,
        "sources": [source],
        "excerpts": excerpts,
...
def _source_record(turn_id: str, text: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    source["receipt_id"] = receipt(
        "source-preserved", source_id, [], {"sha256_source": stable_id("txt", text)}
    )["id"]
    return source
...
def _statement_record(
    source: dict[str, Any], excerpt: dict[str, Any]
) -> dict[str, Any]:
    statement_id = stable_id(
        "stmt", {"excerpt_id": excerpt["id"], "text": excerpt["text"]}
    )
    ...
    for index, statement in enumerate(statements):
        ...
        obs_receipt = receipt(
            ...
            statement["receipt_ids"],
            {
                ...
                "fact_intake": index < len(fact_candidates),
            },
        )
        observations.append(
            {
                ...
                "statement_id": statement["id"],
                "status": status,
                "claim": fact_candidates[index]
                if index < len(fact_candidates)
                else None,
...
def _predicate_records(
    ...
    if supplied_atoms:
        with _MetricSpan(
            ...
            turn_id=turn_id,
            statement_count=len(statements),
            supplied_atom_count=len(supplied_atoms),
        ):
        ...
        with _MetricSpan(
            ...
            for statement in statements:
                ...
                for atom in projected:
                    atom_id = stable_id(
                        "atom", {"statement_id": statement["id"], "atom": atom}
                    )
                    atom_receipt = receipt(
                        "predicate-atom-projected",
                        atom_id,
                        statement["receipt_ids"],
                        {"statement_id": statement["id"]},
                    )
    ...
    abstentions = []
    with _MetricSpan(
        metrics_callback, "pnf_construction", turn_id=turn_id, atom_count=len(atoms)
    ):
        for atom in atoms:
            pnf_id = stable_id(
                "pnf",
                ...
                abstentions.append(
                    {
                        "id": stable_id(
                            "abstain",
                            {"atom_id": atom["id"], "reason": "no-typed-meet"},
                        ),
                        "status": "abstained",
...
def _project_with_fallback(
    projector: Callable[[str], list[dict[str, Any]]] | None, statement: dict[str, Any]
) -> list[dict[str, Any]]:
...
def _optional_projector() -> Callable[[str], list[dict[str, Any]]] | None:
    ...
    for module_name, function_name in (
        (
            "src.sensiblaw.interfaces.shared_reducer",
...
def _normal_form_optional_fields(atom: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        atom.get("semantic_comparison_mode")
        and atom.get("semantic_comparison_mode") != "exact"
    ):
...
def _relations_to_atoms(bundle: dict[str, Any]) -> list[dict[str, Any]]:
    atoms_by_id = {
        atom["id"]: atom for atom in bundle.get("atoms", []) if isinstance(atom, dict)
    }
            ...
            residuals.append(
                {
                    ...
                    "receipt_ids": sorted(
                        left.get("receipt_ids", []) + right.get("receipt_ids", [])
                    ),
                }
            )
    return residuals
...
def _is_classification_tension(left: dict[str, Any], right: dict[str, Any]) -> bool:
    ...
    if left.get("polarity") == right.get("polarity"):
        return False
    left_subject = _role_value(
        (left.get("roles") or {}).get("agent")
        if isinstance(left.get("roles"), dict)
        else None
    )
    right_subject = _role_value(
        (right.get("roles") or {}).get("agent")
        if isinstance(right.get("roles"), dict)
        else None
    )
    left_theme = _role_value(
        (left.get("roles") or {}).get("theme")
        if isinstance(left.get("roles"), dict)
        else None
    )
    right_theme = _role_value(
        (right.get("roles") or {}).get("theme")
        if isinstance(right.get("roles"), dict)
        else None
    )
    return bool(
...
def _pnf_emission_receipts(
    atoms: list[dict[str, Any]], pnfs: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    ...
    for atom in atoms:
        ...
        emitted_atom = {
            ...
            "qualifiers": atom.get("qualifiers")
            or {"polarity": atom.get("polarity", "positive")},
            "wrapper": atom.get("wrapper")
            or {"status": "structural_projection", "evidence_only": True},
...
def _pnf_residual_receipts(
    ...
    receipt_by_atom = {
        ...
        for item in pnf_emission_receipts
        if item.get("atom_id")
    }
    receipts = []
...
def _promotion_gates(
    ...
    checks = {
        ...
        "excerpt_receipts": bool(excerpts)
        and all(item.get("receipt_id") for item in excerpts),
        "statement_receipts": bool(statements)
        and all(item.get("receipt_ids") for item in statements),
        "observation_receipts": bool(observations)
        and all(item.get("receipt_ids") for item in observations),
        "predicate_atom_receipts": bool(atoms)
        and all(item.get("receipt_ids") for item in atoms),
