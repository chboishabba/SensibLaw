...
def _ground(residual: str) -> str | None:
    ...
    checks = (
        ...
        (
            ("impossible_temporal", "invalid_temporal"),
            "impossible_temporal_scope",
        ),
        (
            ("entity_type", "incompatible_type"),
            "incompatible_entity_type",
        ),
        (
            ("wrong_jurisdiction", "incompatible_jurisdiction"),
            "wrong_jurisdiction",
        ),
        (("excess_authority", "authority_overclaim"), "excess_authority"),
        (
            ("invalid_translation", "translation_noncoextensive"),
            "invalid_translation_transport",
        ),
        (
            ("constraint_violation", "constraint_contradiction"),
            "constraint_violation",
        ),
    )
    return next(
    ...
    def __post_init__(self) -> None:
        ...
        if not all(
            (
                self.document_ref,
                self.proposal_ref,
                self.semantic_coordinate_ref,
            )
        ):
            raise ValueError(
                "candidate assessment requires document, proposal, and coordinate"
            )

    @property
    def assessment_ref(self) -> str:
        return "candidate-assessment:" + canonical_sha256(
...
class SemanticLifecycleResult:
    ...
    def lifecycle_ref(self) -> str:
        return "pnf-semantic-lifecycle:" + canonical_sha256(
            self.to_dict(include_ref=False)
        )
    ...
    for proposal in proposals:
        ...
        for element in elements.get(coordinate, ()):
            ...
            for name in (
                ...
                element_refs = _refs(_get(element, name, ()) or ())
                evidence.update(element_refs)
                observed.update(element_refs)
        ...
        ) in {"authoritative", "world_fact", "legal_truth"}:
            grounds.add("excess_authority")
        applicable = not (
            payload.get("applicable") is False
            or payload.get("validation_state") == "inapplicable"
        )
        declared = _refs(
            (
                *tuple(payload.get("constraint_refs") or ()),
                *tuple(payload.get("applied_constraint_refs") or ()),
            )
        )
        applied: list[str] = []
    ...
    for assessment in assessments:
        ...
        if assessment.outcome == "both":
            ...
            reason = (
                ...
                else "semantic_frontier_unresolved"
            )
            state, reasons = "blocked", (reason,)
        else:
...
def _conflict_index(
    ...
    for residual in reduction_residuals:
        if str(_get(residual, "residual_type", "") or "") != (
    ...
    *,
    ...
    for factor in reduced_factors:
        ...
        conflict_refs = _refs(
            ref
            for proposal_ref in proposal_refs
            for ref in conflicts.get(proposal_ref, ())
        )
        ...
        if conflict_refs:
            ...
            ranked = sorted(
                (
                    (_preference(proposal_by_ref[ref]), ref)
                    for ref in admitted
        ...
        grounds = _refs(
            (
                *(
                    assessment_by_ref[ref].assessment_ref
                    for ref in proposal_refs
                    if ref in assessment_by_ref
                ),
                *(
                    admission_by_ref[ref].receipt_ref
                    for ref in proposal_refs
