...
def _meet_for(
    ...
    vector = TypedCompatibilityVector(
        ...
        contradiction=(
            "observed"
            if assessment.get("compatibility_state") == "held_incompatible"
            else "not_observed"
        ),
    )
    residuals = {
    ...
    for result in results:
        ...
        residuals = {
            *[
                row
                for candidate_set in result.get("candidate_sets") or ()
                for row in candidate_set.get("residuals") or ()
            ],
        ...
        packets.append(
            ExternalReviewPacket(
                ...
                overlap_signal_refs=tuple(
                    sorted(signals_by_demand.get(str(demand["demand_ref"]), ()))
                ),
