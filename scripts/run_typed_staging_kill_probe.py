...
The first coordinator runs real process workers, waits until every result is
committed in ``computed`` state, then pauses before revision allocation.  The
parent kills only that coordinator, waits for its coordinator lease to expire,
and starts a replacement.  The replacement must admit the staged rows without
creating any additional semantic job attempt.
"""
...
class PausedAdmissionWorkerPool(DeterministicAdmissionWorkerPool):
    """Test-only pool that exposes the durable computed/admission boundary."""

    def run_until_idle(self) -> dict[str, Any]:
        # Execute and commit every worker result, deliberately bypassing the
        # deterministic-admission override until the marker has been emitted.
        result = TypedProcessPostgresWorkerPool.run_until_idle(self)
        marker = Path(os.environ["SENSIBLAW_TYPED_STAGE_MARKER"])
        marker.parent.mkdir(parents=True, exist_ok=True)
        marker.touch()
        pause_seconds = float(
            os.environ.get("SENSIBLAW_TYPED_STAGE_PAUSE_SECONDS", "600")
        )
    ...
    *,
    ...
    for ordinal in range(count):
        job = SolverJob(
            ...
            declaration_ref="declaration:typed-kill-probe:v1",
            input_revision=0,
            input_refs=(f"observation:{ordinal}",),
            input_payload={"observation_delta": {"observations": ()}},
            rule_set_revision="rules:typed-kill-probe:v1",
...
def main() -> int:
    ...
    report = {
        "schema_version": "sensiblaw.typed-staging-kill-probe.v1",
        "state": "passed",
        "run_ref": args.run_ref,
