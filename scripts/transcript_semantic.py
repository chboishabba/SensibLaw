...
def _demo_units():
    ...
    return [
        ...
        TextUnit(
            "demo-j2", "demo-journal-1", "text_file", "Mary Jane is Bob's sister."
        ),
        TextUnit(
            "demo-j3", "demo-journal-1", "text_file", "Alice is the guardian of Carol."
        ),
...
def build_transcript_semantic_cli_payload(
    *,
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        _emit_progress(
            ...
            relation_candidate_count=int(
                run_payload.get("relation_candidate_count") or 0
            ),
            promoted_relation_count=int(
                run_payload.get("promoted_relation_count") or 0
            ),
        )
        if cmd == "run":
            payload = run_payload
            ...
            payload = build_transcript_relation_summary(
                conn, run_id=str(run_payload["run_id"]), units=units
            )
            _emit_progress(
                progress_callback,
                "summary_build_finished",
                run_id=str(run_payload["run_id"]),
            )
        else:
            _emit_progress(
            ...
            payload = build_transcript_semantic_report(
                conn, run_id=str(run_payload["run_id"]), units=units
            )
            _emit_progress(
                progress_callback,
    ...
    _emit_progress(
        ...
        "build_finished",
        cmd=cmd,
        run_id=str(payload.get("run_id") or run_payload.get("run_id") or ""),
    )
    return payload
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run/report the deterministic transcript/freeform semantic pipeline."
    )
