...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch each distinct Nat P854 source once under one shared rate limiter, "
            "persist successful bodies by SHA-256, and project transport/integrity "
            "observations back to exact source-support residuals."
        )
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Content-addressed source artifact root. Defaults to "
            "<output-without-.json>.artifacts."
        ),
    )
    ...
    if artifact_dir is None:
        artifact_dir = args.output.with_suffix("").with_name(
            args.output.with_suffix("").name + ".artifacts"
        )
        ...
        json.dumps(
            {
                ...
                "artifact_persisted_count": dispatch["artifact_persisted_count"],
                "content_artifact_persisted_residual_count": dispatch[
                    "content_artifact_persisted_residual_count"
                ],
