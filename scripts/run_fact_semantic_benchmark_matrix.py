...
def _infer_report_identity(
    ...
    if not match:
        return corpus_id, count
    return corpus_id or match.group("corpus_id"), count if count is not None else int(
        match.group("count")
    )
...
def _find_baseline_report(
    baseline_dir: Path, *, corpus_id: str, count: int
...
def _build_drift_summary(
    ...
    expectation = (
        payload.get("expectation_summary")
        if isinstance(payload.get("expectation_summary"), dict)
        else {}
    )
    ...
    if not isinstance(baseline_payload, dict):
        ...
        if class_recall < 1.0:
            review_recommendations.append(
                "Class expectation recall below 1.0; inspect entries with missing expected classes."
            )
        if policy_recall < 1.0:
            review_recommendations.append(
                "Policy expectation recall below 1.0; inspect entries with missing expected policies."
            )
        return None, guardrail_status, review_recommendations

    baseline_refresh = (
        baseline_payload.get("refresh")
        if isinstance(baseline_payload.get("refresh"), dict)
        else {}
    )
    ...
    drift["elapsed_ms"] = {
        ...
        "relative_delta": _relative_delta(elapsed_ms, baseline_elapsed_ms)
...
def _review_targets(
    corpus_id: str, payload: dict[str, object], limit: int = 3
    ...
    if corpus_id == "wiki_revision":
        filtered.sort(
            key=lambda row: (
                ...
                or row.get("missing_expected_policies")
                else 1,
        ...
        filtered.sort(
            key=lambda row: (
                ...
                -(
                    len(list(row.get("unexpected_signal_classes", [])))
                    + len(list(row.get("unexpected_policy_outcomes", [])))
                ),
                0
                if row.get("missing_expected_classes")
                or row.get("missing_expected_policies")
        ...
        filtered.sort(
            key=lambda row: (
                -(
                    len(list(row.get("missing_expected_classes", [])))
                    + len(list(row.get("missing_expected_policies", [])))
                ),
    ...
    for row in filtered[:limit]:
        ...
        if row.get("unexpected_signal_classes") or row.get(
            "unexpected_policy_outcomes"
        ):
        ...
        targets.append(
            {
                ...
                "missing_expected_classes": list(
                    row.get("missing_expected_classes", [])
                ),
                "missing_expected_policies": list(
                    row.get("missing_expected_policies", [])
                ),
                "reasons": reasons,
            }
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run the fact semantic benchmark matrix from a corpus manifest."
    )
    parser.add_argument(
        "--manifest",
        type=Path,
    ...
    for row in corpus_rows:
        ...
        for tier in tiers:
            ...
            baseline_report_path, baseline_payload = _find_baseline_report(
                baseline_dir, corpus_id=corpus_id, count=count
            )
            with tempfile.TemporaryDirectory(
                prefix=f"bench-{corpus_id}-{count}-"
            ) as tmpdir:
                ...
                payload["baseline_report_path"] = (
                    str(baseline_report_path)
                    if baseline_report_path is not None
                    else None
                )
                drift, guardrail_status, review_recommendations = _build_drift_summary(
                ...
                results.append(
                    {
                        ...
                        "refresh_status": (
                            (payload.get("refresh") or {}).get("refresh_status")
