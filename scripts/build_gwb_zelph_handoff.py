...
def _load_timeline_payload(*, timeline_path: Path | None = None) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict) or not isinstance(payload.get("events"), list):
        raise ValueError(
            f"timeline payload at {selected_path} is not in expected format: missing list events"
        )
    return payload
...
def _build_reports(
    *, timeline_payload: dict[str, Any]
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        timeline_out_path.write_text(
            json.dumps(timeline_payload, sort_keys=True, indent=2), encoding="utf-8"
        )
        persist_wiki_timeline_aoo_run(
            db_path=db_path,
            out_payload=timeline_payload,
            timeline_path=timeline_out_path,
        )
        with sqlite3.connect(str(db_path)) as conn:
            ...
            linkage_report = build_gwb_us_law_linkage_report(
                conn, run_id=str(linkage_run["run_id"])
            )
            semantic_report = build_gwb_semantic_report(
                conn, run_id=str(semantic_run["run_id"])
            )
...
def _seed_support_kind(row: dict[str, Any]) -> str:
    for event in row.get("events", []):
        if any(
            str(receipt.get("kind")) == "provenance_cue_broad"
...
def _build_slice(
    ...
    *,
    ...
    for row in semantic_report.get("promoted_relations", []):
        ...
        citations = (
            event_row.get("citations")
            if isinstance(event_row.get("citations"), list)
            else []
        )
    ...
    seed_lookup = {
        str(row.get("seed_id")): row for row in linkage_report.get("per_seed", [])
    }
    selected_seed_rows = []
    ...
    unresolved_mentions = [
        {
            ...
            "review_status": "unresolved_surface",
            "mention_count": row.get("mention_count"),
        }
        for row in semantic_report.get("unresolved_mentions", [])
    ]
    ambiguous_events = [
        {
            "event_id": row.get("event_id"),
        ...
        for row in linkage_report.get("ambiguous_events", [])
    ]
    return {
        "version": ARTIFACT_VERSION,
        "fixture_kind": "public_checked_fixture",
        "timeline_path": str(
            (timeline_payload.get("source_timeline") or {}).get("path")
            or timeline_payload.get("source")
            or "wiki_timeline_gwb.json"
        ),
...
def _build_scorecard(
    ...
    return {
        ...
        "promoted_relation_count": slice_payload["summary"][
            "selected_promoted_relation_count"
        ],
        "matched_seed_lane_count": sum(
            1 for row in seed_rows if row["review_status"] == "matched"
        ),
        "candidate_only_seed_lane_count": sum(
            1 for row in seed_rows if row["review_status"] == "candidate_only"
        ),
        "broad_cue_seed_lane_count": sum(
            1 for row in seed_rows if row["support_kind"] == "broad_cue"
        ),
        "direct_support_seed_lane_count": sum(
            1 for row in seed_rows if row["support_kind"] == "direct"
        ),
        "ambiguous_event_count": slice_payload["summary"]["ambiguous_event_count"],
        "unresolved_surface_count": slice_payload["summary"][
...
def _build_summary_text(slice_payload: dict[str, Any]) -> str:
    ...
    for row in slice_payload["selected_seed_lanes"]:
        lines.append(
            f"- {row['action_summary']}: status={row['review_status']}, support={row['support_kind']}, matched_events={row['matched_event_count']}, candidate_events={row['candidate_event_count']}."
        )
...
def _emit_node(
    ...
    *,
    ...
    for line in (
        ...
        f'{node_id} "canonical_key" "{canonical_key}"',
    ):
        if line not in seen:
            seen.add(line)
            lines.append(line)
...
def _build_facts(slice_payload: dict[str, Any]) -> str:
    ...
    for row in slice_payload["selected_promoted_relations"]:
        ...
        _emit_node(
            ...
            seen,
            subject_id,
            kind=str(subject["entity_kind"]),
            label=str(subject["canonical_label"]),
        ...
        _emit_node(
            ...
            label=str(obj["canonical_label"]),
            canonical_key=str(obj["canonical_key"]),
        )
        for line in (
            f'{subject_id} "{row["predicate_key"]}" {object_id}',
            f'{subject_id} "review_status" "promoted_relation"',
        ):
            if line not in seen:
                seen.add(line)
                lines.append(line)
...
def build_handoff_artifact(
    output_dir: Path, *, timeline_path: Path | None = None
) -> dict[str, Any]:
    ...
    slice_payload["compiler_contract"] = build_gwb_public_handoff_contract(
        slice_payload
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the checked GWB public-entity Zelph handoff artifact."
    )
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        ...
        help="Path to the GWB timeline JSON payload.",
    )
    ...
    print(
        json.dumps(
            build_handoff_artifact(
                Path(args.output_dir).resolve(), timeline_path=timeline_path
            ),
            indent=2,
            sort_keys=True,
        )
