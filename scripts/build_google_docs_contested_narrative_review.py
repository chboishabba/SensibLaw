...
def build_google_docs_contested_narrative_review(
    *,
    ...
    _emit_trace(
        trace_callback,
        "google_docs_run_started",
        affidavit_doc_url=affidavit_doc_url,
        response_doc_url=response_doc_url,
    )
    _emit_progress(
        progress_callback,
        "google_affidavit_fetch_started",
        section="google_docs",
        completed=0,
        total=2,
        message="Fetching affidavit Google Doc.",
    )
    affidavit_text_raw = fetch_google_public_export_text(affidavit_doc_url)
    _emit_progress(
        progress_callback,
        "google_affidavit_fetch_finished",
        section="google_docs",
        completed=1,
        total=2,
        message="Affidavit Google Doc fetched.",
        character_count=len(affidavit_text_raw),
    )
    _emit_progress(
        progress_callback,
        "google_response_fetch_started",
        section="google_docs",
        completed=1,
        total=2,
        message="Fetching response Google Doc.",
    )
    response_text_raw = fetch_google_public_export_text(response_doc_url)
    _emit_progress(
        progress_callback,
        "google_response_fetch_finished",
        section="google_docs",
        completed=2,
        total=2,
        message="Response Google Doc fetched.",
        character_count=len(response_text_raw),
    )
    ...
    _emit_progress(
        progress_callback,
        "google_doc_extract_finished",
        section="google_docs",
        message="Extracted affidavit and response text blocks.",
        affidavit_character_count=len(affidavit_text),
        response_character_count=len(response_text),
    )
    ...
    _emit_progress(
        progress_callback,
        "google_response_units_loaded",
        section="google_docs",
        completed=len(response_units),
        total=len(response_units),
        message="Loaded raw response units.",
    )
    response_units = group_contested_response_units(response_units, affidavit_text)
    _emit_progress(
        progress_callback,
        "google_response_units_grouped",
        section="google_docs",
        completed=len(response_units),
        total=len(response_units),
        message="Grouped contested response units.",
    )
    if trace_level == "verbose":
        _emit_trace(
            trace_callback,
    ...
    _emit_progress(
        progress_callback,
        "google_affidavit_review_started",
        section="google_docs",
        message="Building affidavit coverage review.",
    )
    ...
    _emit_progress(
        progress_callback,
        "google_affidavit_review_finished",
        section="google_docs",
        message="Google Docs contested narrative review written.",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Build an affidavit-style contested narrative review from two public Google Docs."
    )
    ...
    parser.add_argument(
        "--db-path",
        default=None,
        help="Optional sqlite path for persisting the normalized contested-review receiver.",
    )
    parser.add_argument(
        "--write-artifacts",
        action="store_true",
        help="Write bulky JSON/markdown artifacts even when --db-path is provided.",
    )
    parser.add_argument(
