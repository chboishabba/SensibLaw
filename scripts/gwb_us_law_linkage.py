...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Import, run, and report deterministic GWB U.S.-law linkage over the canonical ITIR DB."
    )
    ap.add_argument("--db-path", help="Canonical ITIR sqlite database path.")
    sub = ap.add_subparsers(dest="cmd", required=True)

    imp = sub.add_parser(
        "import-seed", help="Import reviewed GWB U.S.-law seed JSON into the shared DB."
    )
    imp.add_argument(
        "--file", default="SensibLaw/data/ontology/gwb_us_law_linkage_seed_v1.json"
    )

    run = sub.add_parser(
        "run", help="Run deterministic linkage matching against the GWB timeline."
    )
    run.add_argument("--run-id")
    run.add_argument("--timeline-suffix", default="wiki_timeline_gwb.json")

    report = sub.add_parser(
        "report", help="Emit deterministic linkage report for a matched run."
    )
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        if args.cmd == "import-seed":
            ...
            print(
                json.dumps(
                    {"ok": True, "db": str(db_path), **result}, indent=2, sort_keys=True
                )
            )
            return
        if args.cmd == "run":
            result = run_gwb_us_law_linkage(
                conn, run_id=args.run_id, timeline_suffix=args.timeline_suffix
            )
            conn.commit()
            print(
                json.dumps(
