...
_MODE_SAMPLES: dict[str, tuple[str, str, dict[str, object]]] = {
    ...
    "au_legal": (
        ...
        {
            "lexical_projection_mode": "au_legal",
...
def _seed_units_from_corpus(
    path: Path, count: int
) -> tuple[list[TextUnit], list[dict[str, object]]]:
...
def _entry_provenance(entry: dict[str, object]) -> dict[str, object]:
    ...
    provenance["benchmark_expected_classes"] = _string_list(
        entry.get("expected_classes")
    )
    provenance["benchmark_expected_policies"] = _string_list(
        entry.get("expected_policies")
    )
    provenance["benchmark_length_bucket"] = str(
        entry.get("length_bucket") or ""
    ).strip()
    return provenance
...
def _collect_entry_diagnostics(
    ...
    *,
    ...
    for fact in workbench.get("facts", []):
        ...
        sample_fact = {
            "fact_id": fact_id,
            "label": str(fact.get("canonical_label") or fact.get("fact_text") or "")[
                :160
            ],
    ...
    for row in conn.execute(
        ...
        for endpoint_kind, endpoint_id in (
            ...
            if (
                ...
                and endpoint_id in source_entry_id_by_source_id
            ):
    ...
    for entry_id in sorted(entries_by_id):
        ...
        realized_signal_classes = sorted(
            realized_fact_classes_by_entry_id.get(entry_id, set())
        )
        realized_source_signal_classes = sorted(
            realized_source_classes_by_entry_id.get(entry_id, set())
        )
        realized_class_set = set(realized_signal_classes) | set(
            realized_source_signal_classes
        )
        realized_policy_outcomes = sorted(
            realized_policies_by_entry_id.get(entry_id, set())
        )
        missing_expected_classes = sorted(set(expected_classes) - realized_class_set)
        missing_expected_policies = sorted(
            set(expected_policies) - set(realized_policy_outcomes)
        )
        unexpected_signal_classes = sorted(realized_class_set - set(expected_classes))
        unexpected_policy_outcomes = sorted(
            set(realized_policy_outcomes) - set(expected_policies)
        )
        ...
        diagnostics.append(
            {
                ...
                "entry_pass": not missing_expected_classes
                and not missing_expected_policies,
            }
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Benchmark fact-intake semantic materialization across bounded source modes."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional benchmark corpus JSON file under tests/fixtures/fact_semantic_bench.",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional SQLite database path. Defaults to a temp file.",
    )
    parser.add_argument(
        "--deferred",
        action="store_true",
        help="Defer semantic materialization to background.",
    )
    ...
    if args.corpus_file is not None:
        units, corpus_entries = _seed_units_from_corpus(
            args.corpus_file, max(int(args.count), 1)
        )
        payload = build_fact_intake_payload_from_text_units(
            units, source_label=f"benchmark:{args.corpus_file.stem}:{args.count}"
        )
        for source, entry in zip(payload["sources"], corpus_entries):
            source["source_type"] = str(
                entry.get("source_type") or source["source_type"]
            )
            source["provenance"] = _entry_provenance(entry)
    else:
        units, provenance = _seed_units(args.mode, max(int(args.count), 1))
        payload = build_fact_intake_payload_from_text_units(
            units, source_label=f"benchmark:{args.mode}:{args.count}"
        )
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        persist_summary = persist_fact_intake_payload(
            conn, payload, deferred_refresh=args.deferred
        )
    ...
    payload_out = {
        ...
        "zelph": {
            ...
            "facts_serialized_count": workbench.get("zelph", {}).get(
                "facts_serialized_count", 0
            ),
            "inferred_fact_count": workbench.get("zelph", {}).get(
                "inferred_fact_count", 0
            ),
            "rule_status": workbench.get("zelph", {}).get("rule_status"),
        },
        "refresh": {
            "refresh_status": str(refresh["refresh_status"])
            if refresh is not None
            else (
                persist_summary.get("refresh_status")
                if "refresh_status" in persist_summary
                else None
            ),
            "current_stage": str(refresh["current_stage"])
            if refresh is not None and refresh["current_stage"] is not None
            else None,
            "facts_serialized_count": int(refresh["facts_serialized_count"])
            if refresh is not None
            else 0,
            "assertion_count": int(refresh["assertion_count"])
            if refresh is not None
            else 0,
            "relation_count": int(refresh["relation_count"])
            if refresh is not None
            else 0,
    ...
    if corpus_entries:
        payload_out["corpus_summary"] = {
            "source_types": sorted(
                {str(row.get("source_type") or "") for row in corpus_entries}
            ),
            "entry_count": len(
                {
            ...
            "expected_class_count": sum(
                len(list(row.get("expected_classes") or [])) for row in corpus_entries
            ),
            "expected_policy_count": sum(
                len(list(row.get("expected_policies") or [])) for row in corpus_entries
            ),
            "long_entry_count": sum(
                1
                for row in corpus_entries
                if str(row.get("length_bucket") or "") == "long"
            ),
        }
