...
def compile_human_review_timeline(
    ...
    *,
    ...
    for ev in historical_timeline.get("source_event_rows", []):
        date = (
            ev.get("anchor", {}).get("text")
            or ev.get("resolved_historical_date")
            or "unknown"
        )
    ...
    active_ev_ids = {
        e.get("event_id") for e in unfiltered_braid.get("source_event_rows", [])
    }
...
def generate_triage_markdown(packet: dict[str, Any]) -> str:
    ...
    for row in timeline:
        ...
        if corrob == "conflicted":
            ...
            event not in {"", None}
            and gaps != "no_relation"
            and confidence not in {"unknown", "ingest_order_only"}
            and corrob in {"strong", "moderate", "single_source"}
        ):
    ...
    rows_with_fragment_pnf_depth = sum(
        1
        for r in timeline
        if r.get("depth_level")
        in ("fragment_pnf", "sentence_pnf", "document_pnf", "braid_node")
    )
    rows_with_braid_node_depth = sum(
        1 for r in timeline if r.get("depth_level") == "braid_node"
    )
...
def build_human_review_timeline_packet(
    checkpoint_path: Path, review_path: Path, timeline_path: Path, output_dir: Path
) -> dict[str, Any]:
    ...
    packet = compile_human_review_timeline(
        checkpoint_payload, review_payload, historical_timeline
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the GWB human review timeline packet."
    )
    parser.add_argument(
        "--checkpoint-path", required=True, help="Path to broader GWB checkpoint JSON."
    )
    parser.add_argument(
        "--review-path",
        required=True,
        help="Path to content corroboration review JSON.",
    )
    parser.add_argument(
        "--timeline-path",
        required=True,
        help="Path to historical timeline candidate JSON.",
    )
    parser.add_argument(
        "--output-dir", required=True, help="Directory to write review packet to."
    )
