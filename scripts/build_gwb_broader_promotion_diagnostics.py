...
def _seed_support_kind(events: list[dict[str, Any]]) -> str:
    for event in events:
        ...
        if any(
            str(receipt.get("kind")) == "provenance_cue_broad" for receipt in receipts
        ):
            return "broad_cue"
    return "direct"


def _sample_seed_events(
    events: list[dict[str, Any]], limit: int = 3
) -> list[dict[str, Any]]:
    sampled = []
...
def _build_seed_diagnostics(family_runs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    rows = [
        ...
        if any(f["source_family"] != "checked_handoff" for f in row["families"])
    ]
...
def _build_family_summary(family: dict[str, Any]) -> dict[str, Any]:
    ...
    candidate_only_seed_count = sum(
        ...
        for row in per_seed
        if int(row.get("matched_event_count") or 0) == 0
        and int(row.get("candidate_event_count") or 0) > 0
    )
    matched_seed_count = sum(
        1 for row in per_seed if int(row.get("matched_event_count") or 0) > 0
    )
    ...
    return {
        ...
        "relation_candidate_count": int(
            (semantic.get("summary") or {}).get("relation_candidate_count") or 0
        ),
        "promoted_relation_count": int(
            (semantic.get("summary") or {}).get("promoted_relation_count") or 0
        ),
        "text_debug_unavailable_reason": str(
            ((semantic.get("review_summary") or {}).get("text_debug") or {}).get(
                "unavailable_reason"
            )
            or ""
        ),
        "top_unresolved_surfaces": [
            str(row.get("surface_text") or "") for row in unresolved[:8]
        ],
...
def _build_summary(
    family_summaries: list[dict[str, Any]], seed_diagnostics: list[dict[str, Any]]
) -> dict[str, Any]:
    return {
        "source_family_count": len(family_summaries),
        "families_with_matched_seed_support": sum(
            1 for row in family_summaries if row["matched_seed_count"] > 0
        ),
        "families_with_relation_candidates": sum(
            1 for row in family_summaries if row["relation_candidate_count"] > 0
        ),
        "families_with_promoted_relations": sum(
            1 for row in family_summaries if row["promoted_relation_count"] > 0
        ),
...
def _build_summary_text(payload: dict[str, Any]) -> str:
    ...
    for row in payload["source_family_summaries"]:
        ...
        if row["text_debug_unavailable_reason"]:
            lines.append(
                f"  text_debug_unavailable_reason: {row['text_debug_unavailable_reason']}"
            )
    lines.extend(["", "## Practical reading", ""])
    lines.append("- The immediate blocker is not source availability.")
    lines.append(
        "- The immediate blocker is lack of text-rich semantic events with defensible anchors in broader-source material."
    )
    lines.append(
        "- The next repair should target event shaping and semantic anchoring diagnostics before any promotion-policy loosening."
    )
    lines.append("")
    return "\n".join(lines)
...
def build_diagnostics(
    ...
    _emit_progress(
        ...
        completed=0,
        total=2,
        message="Building broader promotion diagnostics.",
    )
    family_runs = [
    ...
    _emit_progress(
        progress_callback,
        "family_runs_finished",
        section="diagnostics",
        completed=2,
        total=2,
        message="Family reports built.",
    )
    ...
    _emit_progress(
        ...
        "diagnostics_written",
        section="diagnostics",
        message="Diagnostics artifact written.",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build diagnostics for broader GWB promotion failures."
    )
    parser.add_argument(
        "--output-dir",
