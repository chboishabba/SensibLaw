...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Eagerly rewrite wiki timeline runs into the canonical ITIR DB."
    )
    p.add_argument(
        "--source-db", default="SensibLaw/.cache_local/wiki_timeline_aoo.sqlite"
    )
        ...
        with sqlite3.connect(str(source_db)) as source_conn:
            ...
            run_ids = [
                ...
                for r in source_conn.execute(
                    "SELECT run_id FROM wiki_timeline_aoo_runs ORDER BY generated_at, run_id"
                )
            ]
            for run_id in run_ids:
                ...
                try:
                    ...
                    raise SystemExit(
                        f"failed migrating run_id={run_id} timeline_path={timeline_path}: {exc}"
                    ) from exc
    ...
    for artifact in args.artifact_json:
        ...
        migrated_runs.append(
            {"run_id": res.run_id, "events": res.n_events, "source": str(artifact_path)}
        )
    ...
    with sqlite3.connect(str(dest_db)) as conn:
        ...
        missing_suffixes = [
            ...
            if _pick_best_run(conn, suffix) is None
        ]
